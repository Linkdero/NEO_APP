...
if (function_exists('verificar_session') && verificar_session() == true) :
    ...
    if($filtro == 0){
      if($u->hasPrivilege(302)){
        $listado = $clased->get_reporte_por_fase($fecha,$fecha,3);
        $tipo = 3;
      }else if($u->hasPrivilege(308)){
        $listado = $clased->get_reporte_por_fase($fecha,$fecha,2);
        $tipo = 2;
      }else if(usuarioPrivilegiado()->hasPrivilege(311)){
        $listado = $clased->get_reporte_por_fase($fecha,$fecha,1);
        $tipo = 1;
      }
    }else if($u->hasPrivilege(301)){
      $listado = $clased->get_reporte_por_fase($fecha,$fecha,$filtro);
      $tipo = $filtro;
    }
