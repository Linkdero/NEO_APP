...
class Boleta
{
    ...
    function get_boleta_by_id($vac_id)
    {
        $pdo = Database::connect_sqlsrv();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT  V.emp_dir, D2.descripcion AS dir_des,  D.*
		,V.dia_id, F.id_persona, V.vac_id, V.vac_fch_tra, V.vac_fch_sol, V.vac_fch_ini, V.vac_fch_fin, V.vac_fch_pre, A.anio_des, EV.est_id, EV.est_des, vac_dia, vac_dia_goz, vac_sub, vac_sol, vac_pen, vac_obs, dia_est, nombre, dir_funcional, F.p_funcional, F.p_nominal, F.dir_general, F.id_dirg, F.id_dirfn, F.id_secre, F.id_subsecre
    ...
    function get_all_solicitudes($tipo, $dir, $subsec, $sec, $fff1, $fff2)
    {
        ...
        if ($tipo == 1) {
            ...
            $sql .= "EV.est_id = 5 AND (CONVERT(DATE, V.vac_fch_ini) >=  CAST('" . $current_year . "' AS DATE) OR  CONVERT(DATE, V.vac_fch_fin) >=  CAST('" . $current_year . "' AS DATE))";
        } else if ($tipo == 5) {
            $sql .= " CAST(V.vac_fch_ini AS DATE) BETWEEN CONVERT(DATE, V.vac_fch_ini) AND CONVERT(DATE, V.vac_fch_fin) AND EV.est_id = 5";
        }
        if ($dir != 0) {
          if ($dir == 663 || $dir == 667) { //despacho del secretario
            $direcciones = '(7,2,655,657)';
            $sql .= " AND F.id_direc IN " . $direcciones;
          }else{
...
class Horario
{
    static function get_horario_general($month, $year, $dir, $month1, $year1)
    {
        ...
        if ($dir != 'TODOS') {
            ...
            FROM (SELECT DISTINCT(id_persona), MAX(fecha) AS fecha
                        ...
                        WHERE id_punto in (2,3) AND CONVERT(DATE,fecha) BETWEEN CAST(" . $date1 . " AS DATE) AND CAST(" . $date2 . " AS DATE)
    ...
    function get_tarde_by_empleado_1($id_persona, $month, $year)
    {
        ...
        //         WHERE t.id_punto in (2,3) AND t.id_persona = " . $id_persona . " AND (t.fecha BETWEEN CONVERT(DATE," . $date1 . ") AND CONVERT(DATE," . $date2 . ") )) AS H
    ...
    function get_horario_empleado($id_punto, $id_persona, $month, $year, $month1, $year1)
    {
        ...
        FROM (SELECT
                ...
                convert(date,t.fecha) fecha
                FROM tbl_control_ingreso t
                WHERE t.id_punto in (?,?) AND t.id_persona = ? AND (t.fecha BETWEEN CONVERT(DATE," . $t1 . ") AND CONVERT(DATE," . $t2 . ") )) as H
                GROUP BY H.fecha
                ORDER BY H.fecha";
        ...
        //         LEFT JOIN  [SAAS_APP].[dbo].[tbl_control_horario] AS ch ON ph.id_horario=ch.id_horario"
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array(2,3, $id_persona, $year, $month));
        $horarios = $stmt->fetchAll(PDO::FETCH_ASSOC);
        Database::disconnect_sqlsrv();
        return $horarios;
    }
...
tch.hora_fin, tch.dia, tch.estado, tch.entrada, tch.salida
    ...
    function get_empleados_by_dir($id_persona)
    {
        ...
        $sql = "SELECT DISTINCT F.id_persona, F.nombre, F.p_funcional, F.p_nominal, P.id_status,
                ...
                LEFT JOIN rrhh_empleado E ON F.id_persona = E.id_persona
                LEFT JOIN rrhh_empleado_plaza P ON P.id_empleado = E.id_empleado
                WHERE F.estado = 1 AND P.id_status = 891 AND id_direc = (SELECT id_direc FROM xxx_rrhh_Ficha WHERE id_persona = ?)";
        $stmt = $pdo->prepare($sql);
        if ($stmt->execute(array($id_persona))) {
            $response =  $stmt->fetchAll(PDO::FETCH_ASSOC);
        } else {
    ...
    function get_autoriza_for_permiso()
    {
        ...
        ,CASE WHEN F.id_tipo = 2 THEN F.dir_funcional
            ...
            WHERE F.id_persona IN (30,32,52 ,79 ,88 ,102 ,108 ,117 ,135 ,149 ,166 ,177 ,279 ,355 ,456 ,523 ,537 ,614 ,6640 ,7399 ,8341 ,8347 ,8349 ,8351 ,8352 ,8354 ,8356 ,8357 ,8358 ,8362 ,8366
              ,8370 ,8371 ,8423 ,8428 ,8461 ,8500 ,8511, 7370, 8433, 8423, 8449, 501, 8566, 8344,8594,8835,4);";
        $stmt = $pdo->prepare($sql);
        if ($stmt->execute()) {
            $response =  $stmt->fetchAll(PDO::FETCH_ASSOC);
        } else {
    ...
    function get_horario_diario($fecha, $direccion)
    {
        ...
        INNER JOIN( SELECT id_persona, COUNT(id_persona) as cuenta
                    FROM tbl_control_ingreso t WHERE t.id_punto in (2,3) AND convert(date, t.fecha) = ?
        ...
        INNER JOIN ( SELECT MIN(entrada) entrada, MIN(almuerzo) entra_alm, MAX(almuerzo) sale_alm, MAX(entrada) salida, id_persona
                    FROM (
                            SELECT CASE WHEN convert(time,t.fecha) between '00:00:00' AND '23:59:59' THEN convert(time,t.fecha) END entrada,
                                   ...
                                   FROM tbl_control_ingreso t WHERE t.id_punto in (2,3) AND convert(date, t.fecha) = ?) as H GROUP BY H.id_persona) as R ON F.id_persona = R.id_persona
                                   {$str_where}
