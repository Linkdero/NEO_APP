...
if (function_exists('verificar_session') && verificar_session() == true) :
    ...
    foreach($datos AS $d){
      ...
      $moves = array();
      $moves = $clase->get_all_movimientos_solvencia_by_bodega($bodega,$ini,$fin);
      $data = array();
      ...
      foreach ($moves as $m){
        ...
        $accion.='<button class="btn btn-sm btn-personalizado outline" onclick="reporte_solvencia('.$m['id_doc_solvencia'].')"><i class="fa fa-print"></i></button>';
        $accion.='</div></div></div>';
        $sub_array = array(
          'transaccion'=>$m['id_doc_solvencia'],
          'movimiento'=>$m['tipo_solvencia'],
          //'producto'=>$m['producto'],
