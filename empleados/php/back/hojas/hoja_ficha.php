...
if (function_exists('verificar_session') && verificar_session() == true) :
  ...
  if($tipo['id_contrato']==7 || $tpersona['id_status']==2312 || $tpersona['id_status']==5611){
    ...
    $cont = $clase->get_contrato_actual_by_persona($id_persona);
      if($cont['tipo_contrato'] == 9){
        ...
        $sueldo = $cont['monto_mensual'];//$cont['monto_contrato'] / $meses;
      }
      if($cont['tipo_contrato'] == 8){
        $sueldo = $cont['monto_mensual'] * 30;
      }

      $puestofunc = (!empty($cont['puestofunc'])) ? $cont['puestofunc'] : $cont['puestof'];
      $cargoo = ($cont['tipo_contrato'] == 8) ? 'TALLERISTA' : 'ASESOR';
      $tipo_servicio = (!empty($cont['id_tipo_servicio'])) ? $cont['tipo_servicio'] : '';
      $data2 = array(
        'meses'=>$meses,
        'id_secretaria_funcional'=>(!empty($cont['id_secretaria_funcional']))?$cont['id_secretaria_funcional']:'Sin asignación',
    	  ...
    	  'id_direccion_funcional'=>(!empty($cont['id_direccion_funcional']))?$cont['id_direccion_funcional']:'-- -- -- --',
    		...
    		'direccionfun'=>$cont['direccionf'],//(!empty($c['direccionf']))?$c['direccionf']:'adfadsf',
        ...
        'cargo'=>(!empty($tipo_servicio)) ? $tipo_servicio : $cargoo,
  ...
  $data1 = array(
    ...
    'dependencia'=>(!empty($dlaborales['direccionnficha'])) ? $dlaborales['direccionnficha'] : $cont['direccionf'],
    'cargo'=>(!empty($dlaborales['pueston'])) ? $dlaborales['pueston'] : '',
    'dep'=>$direccionf,//(!empty($dlaborales['departamentof'])) ? $dlaborales['departamentof'] : $direccionf,
