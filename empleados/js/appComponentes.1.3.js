...
Vue.component("detalle-asignacion-plaza", {
  ...
  <div class="row" v-if="empleado.secretarian !='Sin asignación' || empleado.estado == 2312 || empleado.estado == 9102">
    ...
    <div class="col-sm-6" v-if="empleado.tipo_persona == 1052 || empleado.tipo_persona == 9103">
      <button class="btn btn-sm btn-danger" @click="finalizarApoyo(empleado.tipo_persona)"><i class="fa fa-times-circle"></i> Finalizar</button>
    </div>
  </div>
  ...
  methods:{
    ...
    finalizarApoyo: function(tipop){

      var message = (tipop == 1052) ? 'apoyo' : 'practicas';
      var respuesta = (tipop == 1052) ? 'Apoyo finalizado' : 'Practicas finalizadas';
      Swal.fire({
        title: '<strong>¿Desea finalizar a la persona de '+ message +'?</strong>',
        text: "",
        type: 'question',
        showCancelButton: true,
        ...
        if (result.value) {
          ...
          $.ajax({
            ...
            success:function(data){
              Swal.fire({
                type: 'success',
                title: respuesta, //'Apoyo finalizado',
                showConfirmButton: false,
                timer: 1100
              });
              $('#id_cambio').val(1);
      ...
      <table class="table table-sm table-striped table-bordered table-perfil" width="100%">
        ...
        <tbody>
          <tr v-for="f in familiares">
            <td class="text-center"><span v-if="f.flag_fallecido == 1" class="text-info fa-regular fa-tombstone"></span> {{ f.parentesco }}</td>
          ...
          <div class="card-body card-body-white">
            <form class="jsValidacionCrearFamiliar" id="formNuevoFamiliar" :class="isDisabled">
              ...
              <div class="row" v-if="privilegio.bienestar == true && dFamiliar.flag_fallecido == 0">
                <span class="btn btn-sm btn-soft-danger" @click="setDead(dFamiliar.id_referencia)"><i class="fa-regular fa-tombstone"></i> Establecer Facellido</span>
              </div>
              <div class="row" v-if="privilegio.reclu == true || privilegio.acciones == true">
...
Vue.component('detalle-cursos', {
  ...
  <div>
    <div class="card-body ">
      <div class="row" style="width:100%; border: 1px solid #eaf2f9;">
        <div class="table-perfil" style="width:100%">
          <table class="table table-sm table-striped table-bordered" width="100%" >
            <thead class="thead-primary">
              <tr>
                <th style="border-top: none;" class="text-left" colspan="7">
                  <div class="row">
                    <div class="col-sm-4">
                      <i class="u-icon u-icon--sm bg-info text-white rounded-circle mr-3 fa fa-chalkboard"></i> Cursos de Capacitación
                    </div>
                    <div class="col-sm-4">
                      <input class="form-control form-control-sm" type="text" placeholder="Filtrar información" v-model="filter" />
                    </div>
                  </div>
                </th>
              </tr>
        ...
        <div class="tableFixHead ">
          <table class="table table-sm table-striped table-bordered" style="border-radius:2px; max-height: 500px; overflow-y: scroll;">
            <thead >
              <tr>
                <th class="text-center" width="25%" style="background-color: #fff">Centro de Capacitación</th>
                <th class="text-center" width="25%" style="background-color: #fff">Patrocinador</th>
                <th class="text-center" style="background-color: #fff">Tipo</th>
                <th class="text-center" width="25%" style="background-color: #fff">Curso</th>
                <th class="text-center" style="background-color: #fff">Inicio</th>
                <th class="text-center" style="background-color: #fff">Fin</th>
                <th class="text-center" style="background-color: #fff">Modalidad</th>
                <th class="text-center" style="background-color: #fff">Acción</th>
                <th class="text-center" style="background-color: #fff">Validar</th>
              </tr>

            </thead>
            <tbody class="vscrollable">
              <tr v-for="(row, index) in filteredRows">
                <td v-html="highlightMatches(row.nombre_centro_capacitacion)"></td>
                <td v-html="highlightMatches(row.nombre_patrocinador)"></td>
                <td v-html="highlightMatches(row.nombre_tipo_curso)"></td>
                <td v-html="highlightMatches(row.nombre_curso)"></td>
                <td v-html="highlightMatches(row.fecha_inicio)"></td>
                <td v-html="highlightMatches(row.fecha_fin)"></td>
                <td v-html="highlightMatches(row.modalidad)"></td>
                <td class="text-center"><span class="btn btn-sm btn-soft-info" @click="getCursoById(row.arreglo)"><i class="fa fa-pencil-alt"></i></span></td>

                <td class="text-center" style="vertical-align: middle;">
                  <div class="btn-group" v-if="row.id_status==1 && privilegio.desarrollo == true">
                    <span class="btn-soft-info btn-sm btn" @click="validaCurso(row.id_capacitacion,2)"><i class="fa fa-check-circle"></i>
                    </span>
                    <span class="btn-soft-danger btn-sm btn" @click="validaCurso(row.id_capacitacion,3)"><i class="fa fa-trash-alt"></i>
                    </span>
                  </div>
                  <div v-else-if="row.id_status==2">
        ...
        <div v-if="privilegio.desarrollo == true">
          <br>
          <div class="row">
            <div class="col-sm-6 text-left">
              Cursos: {{ cursos.length }} - Cursos filtrados: {{ filteredRows.length }}
            </div>
          ...
          <div class="card shadow-card">
            ...
            <div class="card-body">
              <form class="jsValidacionCapacitacion" id="formValidacionCapacitacion">
                <div class="row">
                  <input id="id_persona" name="id_persona" :value="id_persona" hidden></input>
                  <input id="id_capacitacion" name="id_capacitacion" :value="dCurso.id_capacitacion" hidden></input>
                  <input id="tipo_accion" name="tipo_accion" :value="opc" hidden></input>
                  <!--inicio-->
                  <div class="col-sm-12">
                    <div class="form-group">
                      ...
                      <div class="col-sm-6">
                        <div class="form-group">
                          <div class="">
                            <div class="">
                              <div class=" input-group has-personalizado" >
                                <label class="css-input switch switch-success"><input class="chequeado" id="id_presencial" v-model="picked" name="picked" type="radio" :value="1" required/><span></span> Presencial</label>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="col-sm-6">
                        <div class="form-group">
                          <div class="">
                            <div class="">
                              <div class=" input-group has-personalizado" >
                                <label class="css-input switch switch-success"><input class="chequeado" id="id_virtual" v-model="picked" name="picked" type="radio" :value="2" required/><span></span> Virtual</label>
                              </div>
                            </div>
                          </div>
                        </div>
  ...
  computed:{
    filteredRows(){
      return this.cursos.filter(row => {
        const nombre_centro_capacitacion = row.nombre_centro_capacitacion.toString().toLowerCase();
        const nombre_patrocinador = row.nombre_patrocinador.toString().toLowerCase();
        const nombre_tipo_curso = row.nombre_tipo_curso.toString().toLowerCase();
        const nombre_curso = row.nombre_curso.toString().toLowerCase();
        const fecha_inicio = row.fecha_inicio.toString().toLowerCase();
        const fecha_fin = row.fecha_fin.toString().toLowerCase();
        const modalidad = row.modalidad.toString().toLowerCase();

        const searchTerm = this.filter.toLowerCase();

        return (
          nombre_centro_capacitacion.includes(searchTerm) ||
          nombre_patrocinador.includes(searchTerm) ||
          nombre_tipo_curso.includes(searchTerm) ||
          nombre_curso.includes(searchTerm) ||
  ...
  methods: {
    ...
    getCursoById: function(curso){
      ...
      this.picked = "";
      if(this.opc == 3){
        this.picked = this.dCurso.id_modalidad;
      }else{
        this.picked = "";
      }
      //this.getCentrosCapacitaciones();
    ...
    validaCurso: function(id_capacitacion, estado){
      //inicio
      var instancia = this;
      var title='¿Desea validar este curso?', msg='¡Si validar!', color='#28a745';
      if(estado==3){
        title='¿Desea anular este curso?', msg='¡Si anular!', color='#d33';
      }
      //inicio
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            type: "POST",
            url: "empleados/php/back/persona/action/validar_capacitacion.php",
            data: {
              id_capacitacion,
              estado
            }, //f de fecha y u de estado.
...
//formulariio nominal
Vue.component("formulario-nominal", {
  name:'formulario',
  props: ["row1","row2", "arreglo","tipo_accion","tipo"],
  template: `
  <div class='row'>
    ...
    <div class="form-group">
      <div class="">
        <div class="">
          <label> Secretaría:</label>
          <div class=" input-group  has-personalizado" >
          <select id="idSecretariaNo" name="idSecretariaNo" v-model='idSecretaria' class='form-control form-control-sm' disabled>
            <option v-for='data in secretarias' :value='data.id_direccion'>{{ data.direccion_string }}</option>
          </select>
          </div>
        </div>
    ...
    <div class="form-group">
      <div class="">
        <div class="">
          <label>Sub Secretaría:*</label>
          <div class=" input-group  has-personalizado" >
            <select id="idSubSecretariaNo" name="idSubSecretariaNo" class='form-control form-control-sm' v-model='idSubSecretaria' @change='getDirecciones()' required>
              <option v-for='data in subSecretarias' :value='data.id_direccion'>{{ data.direccion_string }}</option>
            </select>
          </div>
        </div>
    ...
    <div class="form-group">
      <div class="">
        <div class="">
          <label>Dirección:</label>
          <div class=" input-group  has-personalizado" >
            <select id="idDireccionNo" name="idDireccionNo" class='form-control form-control-sm' v-model='idDireccion' @change='getSubDirecciones()'>
              <option v-for='data in direcciones' :value='data.id_direccion'>{{ data.direccion_string }}</option>
            </select>
          </div>
        </div>
    ...
    <div class="form-group">
      <div class="">
        <div class="">
          <label>Sub dirección:</label>
          <div class=" input-group  has-personalizado" >
          <select id="idSubDireccionNo" name="idSubDireccionNo" class='form-control form-control-sm' v-model='idSubDireccion' @change='getDepartamentos()'>
            <option v-for='data in subDirecciones' :value='data.id_direccion'>{{ data.direccion_string }}</option>
          </select>
          </div>
        </div>
    ...
    <div class="form-group">
      <div class="">
        <div class="">
          <label>Departamento:</label>
          <div class=" input-group  has-personalizado" >
          <select id="idDepartamentoNo" name="idDepartamentoNo" class='form-control form-control-sm' v-model='idDepartamento' @change='getSecciones()'>
            <option v-for='data in departamentos' :value='data.id_direccion'>{{ data.direccion_string }}</option>
          </select>
          </div>
        </div>
    ...
    <div class="form-group">
      <div class="">
        <div class="">
          <label>Sección:</label>
          <div class=" input-group  has-personalizado" >
          <select id="idSeccionNo" name="idSeccionNo" class='form-control form-control-sm' v-model='idSeccion'>
            <option value='0' >Seleccionar Seccion</option>
            <option v-for='data in secciones' :value='data.id_direccion'>{{ data.direccion_string }}</option>
          </select>
          </div>
    ...
    <div class="form-group">
      <div class="">
        <div class="">
          <label>Nivel:</label>
          <div class=" input-group  has-personalizado">
          <select id="idNivelNo" name="idNivelNo" class='form-control form-control-sm' v-model="nivelF" required>
            <option v-for='data in niveles' :value='data.id_direccion'>{{ data.direccion_string }}</option>
          </select>
          </div>
        </div>
  ...
  methods:{
    ...
    establecerCodigos: function(){
      ...
      if(this.tipo_accion == 2){
        ...
        setTimeout(() => {
          console.log(this.arreglo.puesto_n);
          this.idCategoria = this.arreglo.categoria_n;
          this.idSubSecretaria = this.arreglo.subsecretaria_n;
          this.idDireccion = this.arreglo.direccion_n;
          this.idSubDireccion  = this.arreglo.subdireccion_n;
          this.idDepartamento  = this.arreglo.departamento_n;
          this.idSeccion = this.arreglo.seccion_n;
          this.puestoF = this.arreglo.puesto_n;
          this.nivelF = this.arreglo.nivel_n;
