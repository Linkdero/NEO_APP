...
if (function_exists('verificar_session') && verificar_session() == true) :
    ...
    //inicio anular periodo
    if($control_id == 4 || $control_id == 7){
      $sql = "SELECT *
              FROM [app_vacaciones].[dbo].[VACACIONES]
              WHERE emp_id =(SELECT emp_id
                             FROM [app_vacaciones].[dbo].[VACACIONES]
                             WHERE vac_id=?) AND est_id IN (1,2)
                             AND vac_id > ?";
      ...
      foreach ($vacs as $v) {
          if (count($vacs) > 0) {
              $error =  "Debe anular las boletas de mayor a menor\n";
          }
      }
    }
    ...
    if ($control_id == 2) {
      //echo '3';
        ...
        $set0 .= ' UPDATE [APP_VACACIONES].[dbo].[DIAS_ASIGNADOS] SET dia_goz=' . $diasa['dia_goz'] . ' WHERE dia_id =' . $dia_id . ';';
        // este no $set0 .= ' UPDATE [APP_VACACIONES].[dbo].[DIAS_ASIGNADOS] SET dia_goz=' . ($diasa['dia_goz'] + $dia_sol) . ' WHERE dia_id =' . $dia_id . ';';
    }
