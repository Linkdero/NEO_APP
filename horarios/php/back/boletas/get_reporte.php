...
if (function_exists('verificar_session') && verificar_session() == true) :
  ...
  foreach ($solicitudes as $s) {
    ...
    $est_id = $s['est_id'];
    $est_des = $s['est_des'];
    $accion = '<div class="btn-group"><span class="btn btn-sm btn-personalizado " data-toggle="modal" data-target="#modal-remoto-lg" href="horarios/php/front/boletas/ver_solicitud.php?vac_id=' . $vid . '""><i class="far fa-calendar-check" title="Ver Solicitud"></i></span></div>';
    $estado = $BOLETA->set_estado_badge($est_id, $est_des);
    // echo $priv;
    // echo $est_id;
    /*if ($priv == 1 && $est_id == 1) {
      $accion = '<div class="btn-group"><button class="btn btn-sm btn-personalizado " onclick="" disabled><i class="fa fa-check"></i></button>' . '<button class="btn btn-sm btn-personalizado " onclick="anular_vacaciones(' . $vid . ', 4,' . $tipo . ')"><i class="fa fa-times"></i></button>' .  '<span class="btn btn-sm btn-personalizado " data-toggle="modal" data-target="#modal-remoto-lg" href="horarios/php/front/boletas/ver_solicitud.php?vac_id=' . $vid . '""><i class="fas fa-calendar" title="Ver Solicitud"></i></span>' . '</div>';
    } else if ($priv == 2 && $est_id == 1) {
      $accion = '<div class="btn-group"><button class="btn btn-sm btn-personalizado " onclick="aprobar_vacaciones(' . $vid . ', 2,' . $tipo . ')"><i class="fa fa-check"></i></button>' . '<button class="btn btn-sm btn-personalizado " onclick="anular_vacaciones(' . $vid . ', 4,' . $tipo . ')"><i class="fa fa-times"></i></button>' .  '<span class="btn btn-sm btn-personalizado " data-toggle="modal" data-target="#modal-remoto-lg" href="horarios/php/front/boletas/ver_solicitud.php?vac_id=' . $vid . '""><i class="fas fa-calendar" title="Ver Solicitud"></i></span>' . '</div>';
    } else if ($priv == 3 && $est_id == 2) {
      $accion = '<div class="btn-group"><button class="btn btn-sm btn-personalizado " onclick="aprobar_vacaciones(' . $vid . ', 5,' . $tipo . ')"><i class="fa fa-check"></i></button>' . '<button class="btn btn-sm btn-personalizado " onclick="anular_vacaciones(' . $vid . ', 7,' . $tipo . ')"><i class="fa fa-times"></i></button>' .  '<span class="btn btn-sm btn-personalizado " data-toggle="modal" data-target="#modal-remoto-lg" href="horarios/php/front/boletas/ver_solicitud.php?vac_id=' . $vid . '""><i class="fas fa-calendar" title="Ver Solicitud"></i></span>' . '</div>';
    } else if ($priv == 4 && $est_id == 1) {
      $accion = '<div class="btn-group"><button class="btn btn-sm btn-personalizado " onclick="aprobar_vacaciones(' . $vid . ', 2,' . $tipo . ')"><i class="fa fa-check"></i></button>' . '<button class="btn btn-sm btn-personalizado " onclick="anular_vacaciones(' . $vid . ', 4,' . $tipo . ')"><i class="fa fa-times"></i></button>' .  '<span class="btn btn-sm btn-personalizado " data-toggle="modal" data-target="#modal-remoto-lg" href="horarios/php/front/boletas/ver_solicitud.php?vac_id=' . $vid . '""><i class="fas fa-calendar" title="Ver Solicitud"></i></span>' . '</div>';
    } else if ($priv == 4 && $est_id == 2) {
      $accion = '<div class="btn-group"><button class="btn btn-sm btn-personalizado " onclick="aprobar_vacaciones(' . $vid . ', 5,' . $tipo . ')"><i class="fa fa-check"></i></button>' . '<button class="btn btn-sm btn-personalizado " onclick="anular_vacaciones(' . $vid . ', 7,' . $tipo . ')"><i class="fa fa-times"></i></button>' .  '<span class="btn btn-sm btn-personalizado " data-toggle="modal" data-target="#modal-remoto-lg" href="horarios/php/front/boletas/ver_solicitud.php?vac_id=' . $vid . '""><i class="fas fa-calendar" title="Ver Solicitud"></i></span>' . '</div>';
    }*/
    $pendientes = 2;// $BOLETA->dias_horas($s['vac_sol'], 2);
    $today = date('Y-m-d');
    $diares = ($s['vac_fch_ini'] < $today) ? (($s['diares']) . "<br><span class='btn btn-sm btn-personalizado ' data-toggle='modal' data-target='#modal-remoto-lg' href='directorio/php/front/telefonos/detalle_telefonos.php?id_persona={$s['id_persona']}' data-toggle='tooltip' data-placement='top' title='Mostrar contactos'>
