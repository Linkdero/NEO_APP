...
if (function_exists('verificar_session') && verificar_session() == true) :
    ...
    if($tipo_user==1118 || usuarioPrivilegiado_acceso()->accesoModulo(8085)){  // administrador
     ...
     $comidas = array();
     $comidas = $clase->get_repo_alimentos_por_fecha($ini,$fin,$dir_id,$comedor);
    ...
    foreach ($comidas as $comida){
      //if($comida['de_oficina']){
        ...
        $sub_array = array(
          'fecha'=>date('d-m-Y', strtotime($comida['fecha'])),
    ...
    createLog(272, 1237, 'APP_ALIMENTOS.dbo.Asignacion_Alimentos','Generando reporte por fechas del: '. fecha_dmy($ini).' al '.fecha_dmy($fin),'', '');
