...
var viewModelFacturaDetalle = new Vue({
  ...
  methods:{
    ...
    agregarInsumosFactura: function(evt,tipo){

      var insumos =  (tipo == 1 ) ? viewModelFacturaDetalle.insumos : viewModelFacturaDetalle.insumosf;
      if(insumos.length == 0){
        evt.preventDefault();
        Swal.fire({
          type: 'error',
        ...
        //inicio
        if (insumos.find((item) => item.checked == true)) {
          //inicio
          if (insumos.find((item) => item.recibido == 'ERROR')) {
            evt.preventDefault();
            Swal.fire({
              type: 'error',
              title: 'Debe ingresar correctamente la información',
              showConfirmButton: false,
              timer: 1100
            });
          } else {
          ...
          Swal.fire({
            type: 'error',
            title: 'Debe seleccionar al menos 1 insumo.',
            showConfirmButton: false,
            timer: 1100
          });
          //fin
