    ...
    methods: {
          ...
          if(this.privilegio.facturas == true){
            this.tableFacturas = $('#tb_facturas').DataTable({
              ...
              dom:
                ...
                "<'row'<'col-sm-6 text-left'B><'col-sm-6 text-right'f>>" +
                "<'row'<'col-sm-12'tr>>" +
      ...
      eventHeader: function(){
        ...
        if(this.totalChequeados == 0){
          ...
          console.log(total_+' || '+ rows_selected.length);
          if(rows_selected.length == total_){
            instanciaF.totalChequeados = 1;
          }else{
            instanciaF.totalChequeados = 0;
        ...
        //console.log('valor n: '+this.totalChequeados);
  ...
  //console.log(rows_selected);
     ...
     $('input[type="checkbox"]', rows).prop('checked', this.checked);
      ...
      cargarInput: function (tipo) {
        ...
        if (rows_selected.length == 0) {
          ...
          if((tipo == 4 || tipo == 5) && instanciaF.tipoFiltroFa == 0){
            ...
            Swal.fire(
              'Atención!',
              "Esta opción no está disponible en esta fasee!",
              'error'
            );
          }
      ...
      agregarChequeVue: function(orden_id){
        var cheque = '';
        Swal.fire({
          title: '<strong>¿Desea asignar cheque a la Factura ?</strong>',
          text: "Digite el Cheque (Número de Cheque).",
          type: 'question',
          html:
          ...
          inputValidator: function (inputValue) {
            return new Promise(function (resolve, reject) {
              if (inputValue && inputValue.length > 0) {
                ...
                Swal.fire({
                  type: 'error',
                  title: 'Necesita ingregar el Nro. de Cheque',
                  showConfirmButton: true
                });
  ...
  preConfirm: () => {
    return $('#my-select2').val()
          ...
          if (result.value) {
            $.ajax({
              type: "POST",
              url: "documentos/php/back/factura/action/add_cheque.php",
              dataType: 'json',
              data:{
                orden_id: orden_id,
                cheque:cheque
      ...
      getGrafica: function(){
        axios.get('documentos/php/back/graficas/get_chart_facturas', {
          ...
          this.dataChartN = response.data.dias_n;
            ...
            setTimeout(() => {
              ...
              am4core.ready(function() {
                ...
                // Add data
                instanciaF.chartFacturaN.data = this.dataChartN;//[{"cantidad":6,"series":"1 d\u00edas"},{"cantidad":41,"series":"2 dias"},{"cantidad":5,"series":"3dias"},{"cantidad":3,"series":"4 dias"},{"cantidad":4,"series":"5 dias"}];
                // Add and configure Series
