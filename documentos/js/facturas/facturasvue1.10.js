        ...
        setTimeout(() => {
          ...
          var buttons = new $.fn.dataTable.Buttons(instanciaF.tableFacturas, {
            buttons: [
              ...
              'csvHtml5',
              'pdfHtml5'*/
              { extend: 'excel', text: '<i class="fa fa-file-excel"></i>', className: 'alerta_no tooltip_noti btn btn-sm btn-outline-info btn-fact', 'data-tooltip': 'Exportar a excel'}
            ]
     ...
     $('input[type="checkbox"]', rows).prop('checked', this.checked);
      ...
      cargarInput: function (tipo) {
        ...
        if (rows_selected.length == 0) {
          ...
          }else if(tipo == 6 && instanciaF.tipoFiltroFa == 0){
            // inicio
            var imgModal = $('#modal-remoto');
            var imgModalBody = imgModal.find('.modal-content');
            $.ajax({
