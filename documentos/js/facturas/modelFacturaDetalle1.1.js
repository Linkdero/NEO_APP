...
var viewModelFacturaDetalle = new Vue({
  ...
  methods:{
    ...
    agregarInsumosFactura: function(evt){
      ...
      if(viewModelFacturaDetalle.insumos.length == 0){
        evt.preventDefault();
        Swal.fire({
          type: 'error',
        ...
        //inicio
        if (viewModelFacturaDetalle.insumos.find((item) => item.checked == true)) {
          //inicio
          if (viewModelFacturaDetalle.insumos.find((item) => item.recibido == 'ERROR')) {
            evt.preventDefault();
            Swal.fire({
              type: 'error',
