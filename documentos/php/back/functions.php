...
class documento
{
  ...
  static function get_pedidos_remesas_by_secretaria($fecha,$tipo)
  {
    ...
    $campoExtra = ($tipo == 5) ? ', ISNULL(h.ped_tra,0) AS comprado' : '';
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.ped_tra,REPLACE(STR(a.ped_num, 5), SPACE(1), '0') AS ped_num,a.ped_fec, a.ped_obs, c.Dir_com AS direccion, b.Uni_Nom AS departamento,
                    d.descripcion AS estado, e.descripcion AS verificacion,
                    CASE WHEN a.Ped_status IN (8147,8144,8141,8170,9108,9109) THEN 'danger' ELSE 'info' END AS color,
                    CASE
                    WHEN a.Ped_status IN (8156,9108,9109) THEN 15
                    ...
                    ELSE 0 END AS porcentaje, f.id_persona_asignada, ISNULL(g.primer_nombre,'')+' '+ISNULL(g.segundo_nombre, '')+' '+ISNULL(g.tercer_nombre,'')+' '+ISNULL(g.primer_apellido,'')+' '+ISNULL(g.segundo_apellido,'')+' '+ISNULL(g.tercer_apellido,'') AS asignado, a.Ped_urgente,
                    a.Ped_num_interno, a.Pac_id,a.Ped_status $campoExtra
             ...
             if($tipo == 0){
               ...
               if ($u->hasPrivilege(301)){
                 ...
                 $sql0.="AND a.Ped_status IN (8140,8157,8160,9109)";
                  //echo '2';
               }
               else if($u->hasPrivilege(311)){
                 $sql0.="AND a.Ped_status IN (8156,8139,9108) OR ISNULL(a.Ped_status,0) = 0 AND YEAR(a.ped_fec) = $fecha";
                 //echo '1';
               ...
               $sql0.=" INNER JOIN (
                 SELECT COUNT(a.ped_tra) AS pedidos, a.ped_tra
                 FROM docto_ped_pago a
                 WHERE a.status = 1
                 GROUP BY  a.ped_tra
               ) AS h ON h.ped_tra = a.ped_tra
               WHERE YEAR(a.ped_fec) = ? AND ISNULL(h.pedidos,0) > 0";
             }
  ...
  static function get_pedidos_remesas_by_secretaria_tipo($fecha, $tipo)
  {
    ...
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT DISTINCT a.ped_tra,REPLACE(STR(a.ped_num, 5), SPACE(1), '0') AS ped_num,a.ped_fec, a.ped_obs, c.Dir_com AS direccion, b.Uni_Nom AS departamento,
  ...
  static function get_pedidos_remesas_by_direccion($direccion, $tipo, $fecha)
  {
    ...
    $sql0 = "SELECT a.ped_tra,REPLACE(STR(a.ped_num, 5), SPACE(1), '0') AS ped_num,a.ped_fec, a.ped_obs, c.Dir_com AS direccion, b.Uni_Nom AS departamento,
                    ...
                    ISNULL(g.primer_nombre,'')+' '+ISNULL(g.segundo_nombre, '')+' '+ISNULL(g.tercer_nombre,'')+' '+ISNULL(g.primer_apellido,'')+' '+ISNULL(g.segundo_apellido,'')+' '+ISNULL(g.tercer_apellido,'') AS asignado, a.Ped_urgente,
                    a.Ped_num_interno, a.Pac_id, a.Ped_status $campoExtra
             FROM APP_POS.dbo.PEDIDO_E a
             INNER JOIN APP_POS.dbo.UNIDAD b ON a.Uni_cor=b.Uni_cor
             INNER JOIN APP_POS.dbo.DIRECCION c ON b.Dir_cor=c.Dir_cor
    ...
    if ($tipo == 1) {
      ...
      $sql0.="AND a.Ped_status IN (8140,8157,8143,8161,8164,8145,8146,8149,8148,8140,8157,8160) ";
    }else if($tipo == 3){
      $sql0.=" AND a.Ped_status IN (8147,8144,8141) ";
    }
    else  if($tipo == 0){
      $sql0 .= " AND ISNULL(a.Ped_status,0) = 0 ";

    }else if($tipo == 5){
      $sql0 .= " AND ISNULL(h.pedidos,0) > 0";
    }
    $sql0 .= " ORDER BY a.ped_tra DESC";
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
  ...
  static function get_pedido_by_id($id_pedido)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.ped_tra,REPLACE(STR(a.ped_num, 5), SPACE(1), '0') AS ped_num,a.ped_fec, a.ped_obs, b.Dir_cor, ISNULL(a.Ped_justificacion, 0) AS Ped_justificacion,
                    a.persona_id, ISNULL(a.Ped_status,0) AS estado, a.Ped_fop,
                    ISNULL(c.primer_nombre,'')+ ' '+ ISNULL(c.segundo_nombre,'')+ ' '+ ISNULL(c.tercer_nombre,'')+ ' '+ ISNULL(c.primer_apellido,'')+ ' '+ ISNULL(c.segundo_apellido,'')+ ' '+ ISNULL(c.tercer_apellido,'') AS empleado
             FROM APP_POS.dbo.PEDIDO_E a
             INNER JOIN APP_POS.dbo.UNIDAD b ON a.Uni_cor=b.Uni_cor
             LEFT JOIN rrhh_persona c ON a.persona_id = c.id_persona
             WHERE a.ped_tra=?
            ";
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
  ...
  static function get_insumos_by_pedido($id_pedido)
  {
    ...
    $sql0 = "SELECT a.Ppr_id, a.Pedd_can, b.Ppr_cod, b.Ppr_codPre, b.Ppr_Nom, b.Ppr_Des, b.Ppr_Pres, b.Ppr_Ren,
              ...
              UPPER(LEFT((b.Ppr_Nom+ ' - '+ b.Ppr_Des+ ' - '+ b.Ppr_Pres),1))+LOWER(SUBSTRING((b.Ppr_Nom+ ' - '+ b.Ppr_Des+ ' - '+ b.Ppr_Pres),2,
              LEN((b.Ppr_Nom+ ' - '+ b.Ppr_Des+ ' - '+ b.Ppr_Pres)))) AS caracteres
  ...
  static function get_fase_cotizacion_by_pedido($ped_tra){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT ped_tipo_seguimiento_id FROM docto_pedido_seguimiento_bitacora
    WHERE ped_tra = ? AND ped_tipo_seguimiento_id = ?";
    ...
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array($ped_tra,8148));
    $aprobacion = $q0->fetch();
    Database::disconnect_sqlsrv();
    if($aprobacion['ped_tipo_seguimiento_id'] == 8148){
      return 'true';
    }else{
      return 'false';
    }
  ...
  static function genera_correlativo_pedido_automatico()
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT TOP 1 a.pedido_resolucion_id,
                    a.pedido_serie,
                    a.pedido_resolucion_nm,
                    a.pedido_fecha_autorizado,
                    a.ingresado_por,
                    a.ingresado_en,
                    b.pedido_num_id,
                    b.pedido_num,
                    ...
                    b.ped_utilizado_en,
                    b.ped_utilizado_por
             FROM APP_POS.dbo.PEDIDO_CE a
             INNER JOIN APP_POS.dbo.PEDIDO_CD b ON b.pedido_resolucion_id = a.pedido_resolucion_id
             WHERE b.pedido_status = 0
...
";
  ...
  static function get_insumos_by_id($id_insumo)
  {
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.Ppr_id, a.Ppr_onu, a.Ppr_cod, a.Ppr_codPre, a.Ppr_Nom, a.Ppr_Des, a.Ppr_Pres,a.Med_id,a.Ppr_Ren,a.Ppr_est,b.Med_nom,
    UPPER(LEFT((a.Ppr_Nom+ ' - '+ a.Ppr_Des+ ' - '+ a.Ppr_Pres),1))+LOWER(SUBSTRING((a.Ppr_Nom+ ' - '+ a.Ppr_Des+ ' - '+ a.Ppr_Pres),2,
    LEN((a.Ppr_Nom+ ' - '+ a.Ppr_Des+ ' - '+ a.Ppr_Pres)))) AS caracteres
           FROM APP_POS.dbo.PPR a
           INNER JOIN APP_POS.dbo.MEDIDA b ON a.Med_id = b.Med_id
           WHERE a.Ppr_id=?
  ...
  static function get_bitacora_by_id($ped_tra)
  {
    ...
    $sql0 = "SELECT TOP 1 c.id_item, a.ped_tra, c.descripcion, b.primer_nombre AS pn,b.segundo_nombre AS sn,b.tercer_nombre AS tn,b.primer_apellido AS pa,b.segundo_apellido AS sa,b.tercer_apellido AS ta, a.ped_seguimiento_fecha,
        		      d.primer_nombre AS pne,d.segundo_nombre AS sne, d.tercer_nombre AS tne, d.primer_apellido AS pae,d.segundo_apellido AS sae,d.tercer_apellido tae, a.ped_observaciones, e.persona_user
          FROM docto_pedido_seguimiento_bitacora a
          INNER JOIN rrhh_persona b ON a.persona_id=b.id_persona
          INNER JOIN tbl_catalogo_detalle c ON a.ped_tipo_seguimiento_id=c.id_item
          LEFT JOIN rrhh_persona d ON a.persona_propietario_id=d.id_persona
          LEFT JOIN rrhh_persona_usuario e ON a.persona_id = e.id_persona
          WHERE a.ped_tra =?
  ...
  static function get_accesos($pantalla)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);


            if($pantalla == 7998 || $pantalla == 7999){
              //directores
              $sql0 = "SELECT a.primer_nombre, a.segundo_nombre,
                      ...
                      WHERE b.id_sistema = ?";
              $sql0 .=" AND b.id_status = ? AND c.id_pantalla IN (?,?) AND c.flag_es_menu = ?
                  			GROUP BY  c.flag_es_menu, b.id_status, a.primer_nombre, a.segundo_nombre,
                              a.tercer_nombre,
                              a.primer_apellido,
                              a.segundo_apellido,
                              a.tercer_apellido, a.id_persona, b.id_acceso
                        HAVING  count(DISTINCT c.id_pantalla) = 2";
              $q0 = $pdo->prepare($sql0);
              if($pantalla == 7998){
                $q0->execute(array(8017, 1119, 306, 323, 1 ));
              }
              else if($pantalla == 7999){
                $q0->execute(array(8017, 1119, 302, 305, 1 ));
              }
              $accesos = $q0->fetchAll();
              Database::disconnect_sqlsrv();
            }else{
              $sql0 = "SELECT a.primer_nombre, a.segundo_nombre,
                      ...
                      INNER JOIN tbl_accesos_usuarios_det c ON c.id_acceso=b.id_acceso
                      WHERE b.id_sistema = ?";
              $sql0 .=" AND c.id_pantalla=? AND c.flag_es_menu=? AND b.id_status = ?";
              $q0 = $pdo->prepare($sql0);
              $q0->execute(array(8017, $pantalla, 1, 1119));
              $accesos = $q0->fetchAll();
              Database::disconnect_sqlsrv();
            }
  ...
  static function get_persona_asignada($ped_tra)
  {
    ...
    $sql0 = "SELECT TOP 1 b.id_persona_asignada AS id_persona, a.primer_nombre, a.segundo_nombre,
            ...
            WHERE b.ped_tra = ? --AND b.id_tipo = 3
...
";
  ...
  static function get_actas_compras($year,$tecnico)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.acta_id, a.acta_fecha, a.acta_finalizacion,
            a.id_director,
            ISNULL(b.primer_nombre,'') + ' '+ISNULL(b.segundo_nombre,'') + ' '+ISNULL(b.tercer_nombre,'') + ' '+
            ISNULL(b.primer_apellido,'') + ' '+ISNULL(b.segundo_apellido,'') + ' '+ISNULL(b.tercer_apellido,'') AS director,
            ISNULL(c.primer_nombre,'') + ' '+ISNULL(c.segundo_nombre,'') + ' '+ISNULL(c.tercer_nombre,'') + ' '+
            ISNULL(c.primer_apellido,'') + ' '+ISNULL(c.segundo_apellido,'') + ' '+ISNULL(c.tercer_apellido,'') AS jefe,
            ISNULL(d.primer_nombre,'') + ' '+ISNULL(d.segundo_nombre,'') + ' '+ISNULL(d.tercer_nombre,'') + ' '+
            ISNULL(d.primer_apellido,'') + ' '+ISNULL(d.segundo_apellido,'') + ' '+ISNULL(d.tercer_apellido,'') AS tecnico,
            (STUFF((SELECT CAST(', ' + CONVERT(VARCHAR(10),REPLACE(STR(bb.pedido_num, 5), SPACE(1), '0')) AS VARCHAR(MAX))
                     FROM APP_POS.dbo.PEDIDO_AD aa
            		 INNER JOIN APP_POS.dbo.PEDIDO_CD bb ON aa.ped_tra = bb.ped_tra
                     WHERE (aa.acta_id = a.acta_id)
                     FOR XML PATH ('')), 1, 2, '')) AS pyr,
            		 a.nit_proveedor,e.Prov_nom, a.acta_monto,a.acta_justificacion
            FROM APP_POS.dbo.PEDIDO_A a
            INNER JOIN rrhh_persona b ON a.id_director = b.id_persona
            INNER JOIN rrhh_persona c ON a.id_jefe = c.id_persona
            INNER JOIN rrhh_persona d ON a.id_tecnico = d.id_persona
            INNER JOIN APP_POS.dbo.PROVEEDOR e ON a.nit_proveedor = e.Prov_id
            WHERE YEAR(a.acta_fecha) = ?
            ORDER BY a.acta_id DESC
    ";

    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array($year));
    $actas = $q0->fetchAll();
    Database::disconnect_sqlsrv();
    return $actas;
  }
    ...
    static function get_acta_compra_by_id($acta_id)
    {
      $pdo = Database::connect_sqlsrv();
      $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $sql0 = "SELECT a.acta_id, a.acta_fecha, a.acta_finalizacion,
               ...
               ISNULL(UPPER(LEFT(b.primer_nombre,1))+LOWER(SUBSTRING(b.primer_nombre,2,LEN(b.primer_nombre))) ,'') + ' '+ISNULL(UPPER(LEFT(b.segundo_nombre,1))+LOWER(SUBSTRING(b.segundo_nombre,2,LEN(b.segundo_nombre))) ,'') + ' '+ISNULL(UPPER(LEFT(b.tercer_nombre,1))+LOWER(SUBSTRING(b.tercer_nombre,2,LEN(b.tercer_nombre))) ,'') + ' '+
               ISNULL(UPPER(LEFT(b.primer_apellido,1))+LOWER(SUBSTRING(b.primer_apellido,2,LEN(b.primer_apellido))) ,'') + ' '+ISNULL(UPPER(LEFT(b.segundo_apellido,1))+LOWER(SUBSTRING(b.segundo_apellido,2,LEN(b.segundo_apellido))) ,'') + ' '+ISNULL(UPPER(LEFT(b.tercer_apellido,1))+LOWER(SUBSTRING(b.tercer_nombre,2,LEN(b.tercer_nombre))) ,'')AS director,
               ISNULL(UPPER(LEFT(c.primer_nombre,1))+LOWER(SUBSTRING(c.primer_nombre,2,LEN(c.primer_nombre))) ,'') + ' '+ISNULL(UPPER(LEFT(c.segundo_nombre,1))+LOWER(SUBSTRING(c.segundo_nombre,2,LEN(c.segundo_nombre))) ,'') + ' '+ISNULL(UPPER(LEFT(c.tercer_nombre,1))+LOWER(SUBSTRING(c.tercer_nombre,2,LEN(c.tercer_nombre))) ,'') + ' '+
               ISNULL(UPPER(LEFT(c.primer_apellido,1))+LOWER(SUBSTRING(c.primer_apellido,2,LEN(c.primer_apellido))) ,'') + ' '+ISNULL(UPPER(LEFT(c.segundo_apellido,1))+LOWER(SUBSTRING(c.segundo_apellido,2,LEN(c.segundo_apellido))) ,'') + ' '+ISNULL(UPPER(LEFT(c.tercer_apellido,1))+LOWER(SUBSTRING(c.tercer_nombre,2,LEN(c.tercer_nombre))) ,'') AS jefe,
               ISNULL(UPPER(LEFT(d.primer_nombre,1))+LOWER(SUBSTRING(d.primer_nombre,2,LEN(d.primer_nombre))) ,'') + ' '+ISNULL(UPPER(LEFT(d.segundo_nombre,1))+LOWER(SUBSTRING(d.segundo_nombre,2,LEN(d.segundo_nombre))) ,'') + ' '+ISNULL(UPPER(LEFT(d.tercer_nombre,1))+LOWER(SUBSTRING(d.tercer_nombre,2,LEN(d.tercer_nombre))) ,'') + ' '+
               ISNULL(UPPER(LEFT(d.primer_apellido,1))+LOWER(SUBSTRING(d.primer_apellido,2,LEN(d.primer_apellido))) ,'') + ' '+ISNULL(UPPER(LEFT(d.segundo_apellido,1))+LOWER(SUBSTRING(d.segundo_apellido,2,LEN(d.segundo_apellido))) ,'') + ' '+ISNULL(UPPER(LEFT(d.tercer_apellido,1))+LOWER(SUBSTRING(d.tercer_nombre,2,LEN(d.tercer_nombre))) ,'') AS tecnico,
               (STUFF((SELECT CAST(', ' + CONVERT(VARCHAR(10),REPLACE(STR(bb.pedido_num, 5), SPACE(1), '0')) AS VARCHAR(MAX))
                         FROM APP_POS.dbo.PEDIDO_AD aa
                		 INNER JOIN APP_POS.dbo.PEDIDO_CD bb ON aa.ped_tra = bb.ped_tra
                         WHERE (aa.acta_id = a.acta_id)
                         FOR XML PATH ('')), 1, 2, '')) AS pyr,
                		 a.nit_proveedor,e.Prov_nom, a.acta_monto,a.acta_justificacion,a.id_tipo_pago, a.id_tipo_adjudicacion, a.id_tipo_compra
                FROM APP_POS.dbo.PEDIDO_A a
                INNER JOIN rrhh_persona b ON a.id_director = b.id_persona
                INNER JOIN rrhh_persona c ON a.id_jefe = c.id_persona
                INNER JOIN rrhh_persona d ON a.id_tecnico = d.id_persona
      ...
      //if($e['id_de'])
      $q0 = $pdo->prepare($sql0);
      $q0->execute(array($acta_id));
      $acta = $q0->fetch();
      Database::disconnect_sqlsrv();
      return $acta;
  }
  static function get_pedidos_by_estado($estado){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT d.Ped_tra, REPLACE(STR(d.Ped_num, 5), SPACE(1), '0') AS ped_num, d.Ped_fec, e.descripcion AS estado,
          ...
          INNER JOIN APP_POS.dbo.DIRECCION i ON h.Dir_cor = i.Dir_cor
          --WHERE CONVERT(varchar, f.ped_seguimiento_fecha, 23) BETWEEN ? AND ?
          WHERE YEAR(f.ped_seguimiento_fecha) = ?  ";

          if($estado > 0){
            //$sql0 .= " AND e.ped_tipo_seguimiento_id = ? ";
          }
          if($estado == 8148){
            $sql0 .= " AND ISNULL(d.Ped_justificacion,0) = 0 ";
          }
          $sql0.="GROUP BY d.Ped_tra,d.Ped_num, d.Ped_fec,e.ped_tipo_seguimiento_id,e.descripcion,
          f.ped_seguimiento_fecha,g.ped_seguimiento_fecha,e.ped_observaciones,
          i.Dir_com,d.Ped_obs,d.Ped_justificacion
          ORDER BY f.ped_seguimiento_fecha DESC";

    $q0 = $pdo->prepare($sql0);
    $q0->execute(array(date('Y'),$estado));
    $pedidos = $q0->fetchAll();
    Database::disconnect_sqlsrv();
    return $pedidos;
  }

  static function get_pedidos_seleccionados($valores)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT Ped_tra AS ped_tra, REPLACE(STR(Ped_num, 5), SPACE(1), '0') AS ped_num, Ped_obs, CONVERT(VARCHAR, Ped_fec, 105) AS Ped_fec
             FROM APP_POS.dbo.PEDIDO_E WHERE Ped_tra IN $valores ";
  ...
  static function get_persona_asignada_($ped_tra)
  {
    ...
    $sql0 = "SELECT ISNULL(b.primer_nombre,'')+ ' '+ISNULL(b.segundo_nombre,'')+ ' '+ISNULL(b.tercer_nombre,'')+ ' '+ISNULL(b.primer_apellido,'')+ ' '+ISNULL(b.segundo_apellido,'')+ ' '+ISNULL(b.tercer_apellido,'') AS persona
            ...
            WHERE a.Ped_tra=? AND a.status_id = ? AND a.id_tipo = ?
    ";
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array($ped_tra, 1, 3));
    $per = $q0->fetch();
    Database::disconnect_sqlsrv();
    return $per;
  }
  static function get_facturas($tipo,$tecnico,$cheque,$year)
  {
    $fitro='';
    if($cheque == 1){
      $filtro= " AND ISNULL(a.modalidad_pago,0) IN (1) ";
    }else if($cheque == 0){
      $filtro= " AND ISNULL(a.modalidad_pago,0) IN (0,2) ";
    }else if($cheque == 2){
      $filtro= " AND ISNULL(a.modalidad_pago,0) IN (0,1,2) ";
    }

    $campoExtra = ($tipo == 1 || $tipo == 2) ? ",i.cur_compromiso_pre, i.cur_devengado_pre" : "";
    ...
    $sql0 = "SELECT a.orden_compra_id, a.nro_orden, a.year, a.tipo_pago, a.status, a.asignado_en, a.asignado_por,b.ped_tra, a.reng_num,
                    ...
                    a.password, a.ped_nog AS nog, h.chequeNum AS cheque, a.cur,b.Ped_num,a.cur_devengado,
                    ISNULL(e.primer_nombre, '')+ ' '+ISNULL(e.segundo_nombre, '')+ ' '+ISNULL(e.tercer_nombre, '')+ ' '+ISNULL(e.primer_apellido, '')+ ' '+
                    ISNULL(e.segundo_apellido, '')+ ' '+ISNULL(e.tercer_apellido, '') AS empleado, a.clase_proceso, ISNULL(a.factura_publicada, 0) AS factura_publicada,
                    a.npg, f.descripcion_corta AS estado_factura, a.id_renglon, g.descripcion AS direccion, ISNULL(a.modalidad_pago,0) AS modalidad_pago, a.id_tipo_ingreso
                    $campoExtra
            FROM docto_ped_pago a
            ...
            LEFT JOIN rrhh_direcciones g ON a.id_direccion = g.id_direccion
            LEFT JOIN ChequeDetalle h ON a.cheque_nro = h.chequeId
            ";

            if($cheque == 0 || $cheque == 2){
              if($tipo == 0){//pendientes
                $sql0 .= "WHERE YEAR(a.asignado_en) IN ($year) AND ISNULL(a.modalidad_pago,0) IN (0,1) AND ISNULL(a.factura_publicada,0) = 0 AND a.status = 1";
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
                //$sql0 .= "WHERE YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND ISNULL(a.modalidad_pago,0) = 0 AND ISNULL(a.factura_publicada,0) = 0 AND a.status = 1";
                if($tecnico > 0){
                  $sql0.=" AND d.id_persona = $tecnico";
                }
                $sql0.=$filtro;
                $sql0 .= "OR YEAR(a.asignado_en) IN ($year) AND ISNULL(a.modalidad_pago,0) = 2 AND ISNULL(a.cur,0) = 0 AND ISNULL(a.factura_publicada,0) = 0 AND a.status = 1 ";
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
                //$sql0 .= "OR YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND ISNULL(a.modalidad_pago,0) = 2 AND ISNULL(a.cur,0) = 0 AND ISNULL(a.factura_publicada,0) = 0 AND a.status = 1";
                if($tecnico > 0){
                  $sql0.=" AND d.id_persona = $tecnico ";
                }
                $sql0.=$filtro;
              }else if($tipo == 1 || $tipo == 2){ //liquidadas acreditamiento
                //$sql0 .= " WHERE YEAR(a.asignado_en) IN ($year) AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND a.cur_devengado > 0 AND a.status = 1";
                $sql0 .="LEFT JOIN (
                 SELECT a.cur_compromiso AS cur_compromiso_pre,
                  a.cur_devengado AS cur_devengado_pre, a.id_pago
                 FROM docto_ped_pago_presupuesto a
                 WHERE a.id_status = 1
                 --GROUP BY  a.id_pago
               ) AS i ON i.id_pago = a.id_pago
               WHERE YEAR(a.asignado_en) IN ($year) AND ISNULL(a.modalidad_pago,0) = 2 ";
               $sql0.= ($tipo == 1) ? "AND ISNULL(i.cur_compromiso_pre,0) > 0 AND ISNULL(i.cur_devengado_pre,0) = 0" :"AND ISNULL(i.cur_compromiso_pre,0) > 0 AND ISNULL(i.cur_devengado_pre,0) > 0 ";
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
                //$sql0 .= " WHERE YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND a.cur_devengado > 0 AND a.status = 1";
              }else
              if($tipo == 4){ //liquidadas acreditamiento
                $sql0 .="LEFT JOIN (
                 SELECT a.cur_compromiso AS cur_compromiso_pre,
                  a.cur_devengado AS cur_devengado_pre, a.id_pago
                 FROM docto_ped_pago_presupuesto a
                ...
                WHERE YEAR(a.asignado_en) IN ($year) AND a.factura_publicada = 1 AND ISNULL(i.cur_compromiso_pre,0) = 0 AND ISNULL(i.cur_devengado_pre,0) = 0 AND a.status = 1";
                $sql0.=$filtro;
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
                //$sql0 .= " WHERE YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND a.factura_publicada = 1 AND ISNULL(a.cur,0) = 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.status = 1";
              }else if($tipo == 3){
                $sql0 .= " WHERE YEAR(a.asignado_en) IN ($year) AND a.status = 5";
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
                //$sql0 .= " WHERE YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND a.status = 5";
                $sql0.=$filtro;
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
              ...
              if($tipo == 0){//pendientes
                $sql0 .= "WHERE YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.cheque_nro,0) = 0 AND ISNULL(a.factura_publicada,0) = 0 AND ISNULL(a.id_tipo_ingreso,0) IN (1,2)
                          OR YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.cheque_nro,0) > 0 AND ISNULL(a.factura_publicada,0) = 0 AND ISNULL(a.id_tipo_ingreso,0) IN (1,2)
                          OR YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.cheque_nro,0) = 0 AND ISNULL(a.id_tipo_ingreso,0) IN (3,4)";
              }else
              if($tipo == 4){ //deuda
                $sql0 .= "WHERE YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.factura_publicada,0) = 1 AND ISNULL(a.id_tipo_ingreso,0) IN (1,2)";
                $sql0 .= "OR YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.cheque_nro,0) > 0 AND ISNULL(a.id_tipo_ingreso,0) IN (3,4)";
                //$sql0 .= "WHERE YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.status = 1";
              }else if($tipo == 3){
                $sql0 .= " WHERE YEAR(a.asignado_en) = $year AND a.status = 5 AND ISNULL(a.modalidad_pago,0) = 1 ";
                //$sql0 .= " WHERE YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND a.status = 5";
              }
            }
  ...
  static function get_factura_by_id($orden)
  {
    ...
    $sql0 = "SELECT a.orden_compra_id, a.nro_orden, a.year, a.tipo_pago, a.status, a.asignado_en, a.asignado_por,b.ped_tra, a.reng_num,
                    ...
                    a.cur_devengado,a.regimen_proveedor,a.clase_proceso,
                    ...
                    d.id_persona AS id_tecnico,
                    a.id_direccion, h.chequeNum AS cheque
                    /*,
  ...
  static function get_insumos_by_factura($orden)
  {
    ...
    $sql0 = "SELECT a.orden_compra_id, a.reng_num, a.Ppr_id, a.Ppr_can, a.Ppr_status,
                    ...
                    b.Ppr_Ren, b.Ppr_Nom, b.Ppr_Des, c.Med_nom, b.Ppr_Pres, a.Ppr_costo, a.Ped_tra
  ...
  static function get_estado_by_factura($orden_id){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT TOP 1 a.orden_compra_id,a.id_seguimiento,a.reng_num,a.operado_por,a.operado_en
                ,a.persona_id,a.fac_observaciones,a.fac_status,a.id_seguimiento_detalle,
      				b.descripcion_corta, c.primer_nombre, c.segundo_nombre, c.tercer_nombre,
      				c.primer_apellido, c.segundo_apellido, c.tercer_apellido
                   FROM docto_ped_pago_bitacora AS a
  ...
  static function insert_bitacora_factura($fac_id, $tipo, $persona_id, $observaciones,$estado,$tipo_detalle,$id_direccion){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    // genera correlativo
    ...
    $sql0 = "INSERT INTO docto_ped_pago_bitacora (orden_compra_id, id_seguimiento, reng_num, operado_por, operado_en,
             persona_id, fac_observaciones, fac_status, id_seguimiento_detalle,id_direccion)
    VALUES(?,?,?,?,?,?,?,?,?,?)";
    $q0 = $pdo->prepare($sql0);
    $q0->execute(
      array(
  ...
  static function get_ordenes_pendientes($tipo)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT nro_orden FROM docto_ped_pago
             WHERE YEAR(asignado_en) >= '2022' AND MONTH(asignado_en) > '04'  AND ISNULL(nro_orden,0) > 0 ";

            if($tipo == 1){ //cur de compromiso
              $sql0 .= 'AND ISNULL(cur,0) = 0 AND ISNULL(cur_devengado,0) = 0 ';
            }else
            if($tipo == 2){ //cur devengado
              $sql0 .= 'AND ISNULL(cur,0) > 0 AND ISNULL(cur_devengado,0) = 0 ';
            }
            $sql0.="GROUP BY nro_orden";
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array());
    $facturas = $q0->fetchAll();
  ...
  static function get_ordenes_group_pendientesbk($tipo)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT nro_orden, clase_proceso, year, cur, cur_devengado, SUM(factura_total) AS total, id_status, id_bitacora FROM docto_ped_pago
             WHERE YEAR(asignado_en) >= '2022' AND MONTH(asignado_en) > '04' AND ISNULL(nro_orden,0) > 0 AND status = 1";

            if($tipo == 0){ //sin cur
              $sql0 .= 'AND ISNULL(cur,0) = 0 AND ISNULL(cur_devengado,0) = 0 ';
            }else
            if($tipo == 1){ //cur compromiso
              $sql0 .= 'AND ISNULL(cur,0) > 0 AND ISNULL(cur_devengado,0) = 0 ';
            }
            if($tipo == 2){ //cur devengado
              $sql0 .= 'AND ISNULL(cur,0) > 0 AND ISNULL(cur_devengado,0) > 0 ';
            }
            $sql0.="GROUP BY nro_orden, clase_proceso, year, cur, cur_devengado, id_status, id_bitacora";
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array());
    $facturas = $q0->fetchAll();
  ...
  static function get_ordenes_group_pendientes($tipo,$year,$privilegio,$pen,$apro,$cur_c,$cur_d,$liq,$sinliq,$compras,$presupuesto,$subdirector)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.id_pago, a.nro_registro, a.id_tipo_pago,  a.cur_compromiso, a.cur_devengado, b.total, 0 as nro_orden,
                    g.Prov_nom AS proveedor, a.id_status,
                    d.bitacora_nombre, d.bitacora_color, ISNULL(d.bitacora_icono,'') AS icono,
                    ISNULL(e.bitacora_nombre,'') AS seguimiento_nombre, ISNULL(e.bitacora_color,'') AS seguimiento_color, ISNULL(e.bitacora_icono,'') AS seguimiento_icono,
                    ISNULL(a.id_bitacora,1) AS id_bitacora, ISNULL(a.id_seguimiento,0) AS id_seguimiento,
                    ISNULL(f.primer_nombre,'') + ' '+ISNULL(f.segundo_nombre,'') + ' '+ISNULL(f.tercer_nombre,'') + ' '+ISNULL(f.primer_apellido,'') + ' '+ISNULL(f.segundo_apellido,'') + ' '+ISNULL(f.tercer_apellido,'') AS creador,
                    a.nro_liquidacion, '' AS facturas --gg.facturas
             FROM docto_ped_pago_presupuesto a
             LEFT JOIN (
               SELECT SUM(a.factura_total) AS total, b.idPago
               FROM docto_ped_pago a
               INNER JOIN DoctoPedPagoPresupuestoDetalle b ON b.idFactura = a.orden_compra_id
               WHERE  b.idStatus = 1
               GROUP BY  b.idPago
             ) AS b ON b.idPago = a.id_pago
             LEFT JOIN (SELECT  T.Prov_id, T.Prov_nom, T.id_pago
               FROM
               (
                 SELECT a.id_pago, c.Prov_id, c.Prov_nom
                 FROM docto_ped_pago AS a
                 LEFT JOIN docto_ped_pago_presupuesto AS b ON a.id_pago = b.id_pago
                 LEFT JOIN APP_POS.dbo.PROVEEDOR AS c ON a.proveedor_id = c.Prov_id COLLATE Modern_Spanish_CI_AS
                 GROUP BY c.Prov_id, c.Prov_nom, a.id_pago
               ) T
             ) AS g ON g.id_pago = a.id_pago
             LEFT JOIN DoctoPedPagoPresupuestoCatalogoBit d ON ISNULL(a.id_bitacora,0) = d.bitacora_id
             LEFT JOIN DoctoPedPagoPresupuestoCatalogoBit e ON ISNULL(a.id_seguimiento,0) = e.bitacora_id
             LEFT JOIN rrhh_persona f ON a.creado_por = f.id_persona
      			 LEFT JOIN (
      				SELECT a.id_pago, STRING_AGG(CONVERT(NVARCHAR(max), ISNULL(a.dato,'N/A')), '| ') AS facturas
      				FROM (
      					SELECT b.id_pago, ISNULL(b.factura_serie,'') + '; '+ CONVERT(VARCHAR(15), ISNULL(b.factura_fecha,'')) + '; ' + ISNULL(b.factura_num,'') + '; '+CONVERT(VARCHAR(20),b.factura_total) + '; '+ CONVERT(VARCHAR(15),c.Prov_id) COLLATE Modern_Spanish_CI_AS + '; '+ c.Prov_nom AS dato
      					FROM docto_ped_pago b
      					INNER JOIN APP_POS.dbo.PROVEEDOR c ON b.proveedor_id = c.Prov_id COLLATE Modern_Spanish_CI_AS
      					GROUP BY b.id_pago, b.factura_serie, b.factura_fecha, b.factura_num, b.factura_total, c.Prov_id, c.Prov_nom
      				) AS a
      				WHERE ISNULL(a.id_pago,0) > 0
      				GROUP BY a.id_pago
            ) AS gg ON gg.id_pago = a.id_pago
            WHERE a.id_year = ? ";
            if($year == 2022){
              $sql0 .= "AND MONTH(a.creado_en) > '04' ";
            }
            if($tipo == 0){ //sin cur
              $sql0 .= '';
              if($privilegio['compras_au'] == 'true' || $privilegio['compras_tecnico'] == 'true'){
                //echo '1';
                $sql0.=' AND a.id_status = 1 AND ISNULL(a.id_seguimiento,0) NOT IN (7,16)';
              }else
              if($privilegio['presupuesto'] == 'true')
              {
                //echo '2';
                $sql0.= ' AND a.id_status = 1 AND a.id_bitacora NOT IN (1) AND ISNULL(a.id_seguimiento,0) NOT IN (7,16)';
              }else
              if($privilegio['subdirectorfinanciero'] == 'true'){
                //echo '3';
                $sql0.= ' AND a.id_seguimiento = 14 AND a.id_status = 1';
              }
            }else
            if($tipo == 1){ //cur compromiso
              $sql0 .= 'AND ISNULL(a.cur_compromiso,0) > 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.id_status = 1 ';
            }else
           if($tipo == 2){ //cur devengado
             $sql0 .= 'AND a.id_seguimiento IN (7,16) AND a.id_status = 1 ';
           }else if($tipo == 3){
             $sql0 .= 'AND a.id_status = 3';
           }
           else if($tipo == 4){
             $sql0 .= 'AND a.id_status = 3';
           }
             ...
             /*if($tipo == 0){ //sin cur
               ...
               if($privilegio['compras_au'] == 'true' || $privilegio['compras_tecnico'] == 'true'){
                 //echo '1';
                 $sql0.= "AND a.id_bitacora IN (1) ";
                 $options = '';
                 if($pen == 'true' && $apro == 'false'){
                   $sql0.=" AND a.id_status = 1 AND ISNULL(a.id_seguimiento,0) IN (0) AND id_bitacora NOT IN (2)";
                 }else if($pen == 'false' && $apro == 'true'){
                   $sql0.=" AND a.id_status = 1 AND ISNULL(a.id_seguimiento,0) NOT IN (0,7,16) ";
                 }

                 if($liq == 'true'){
                   $sql0.= " AND ISNULL(a.nro_liquidacion,0) > 0 ";
                 }
                 if($sinliq == 'true'){
                   $sql0.= " AND ISNULL(a.nro_liquidacion,0) = 0 ";
                 }
                 ...
                 if($cur_c == 'true'){
                   $sql0.=" AND ISNULL(a.cur_compromiso,0) > 0";
                 }

                 if($cur_d == 'true'){
                   $sql0.=" AND ISNULL(a.cur_devengado,0) > 0";
                 }
                 //echo $options;
  ...
  static function get_pac_by_ejercicio($dir,$year)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    if($dir > 0){
      $sql0.= "WHERE id_direccion = $dir AND pac_ejercicio_fiscal = $year";
    }else{
      $sql0.= "WHERE pac_ejercicio_fiscal = $year";
    }
  ...
  static function get_pac_by_array($pac_id)
  {
    $arreglo = "(".substr($pac_id,1).")";

    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.pac_id, a.pac_nombre, a.pac_detalle, a.pac_renglon, b.nombre AS renglon_nm,
            a.pac_ejercicio_ant, a.pac_ejercicio_ant_desc, a.pac_creado_por, a.id_status,
            c.descripcion AS direccion
            FROM APP_POS.dbo.PAC_E a
            INNER JOIN APP_POS.dbo.RENGLON b ON a.pac_renglon = b.id_renglon
            INNER JOIN rrhh_direcciones c ON a.id_direccion = c.id_direccion
            WHERE a.pac_id IN $arreglo";
  ...
  // functions bodega
  static function get_productos_man()
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.Pro_idint, a.Bod_id, a.Prd_cos, a.Prd_exi, detalle.TOTAL
FROM APP_POS.dbo.PRODUCTO_DETALLE a
LEFT JOIN (SELECT a.Pro_idint, SUM(a.Envd_can) AS TOTAL FROM APP_POS.dbo.ENVIO_D a
               INNER JOIN APP_POS.dbo.ENVIO_E b ON a.Env_tra = b.Env_tra
               WHERE a.Pro_idint IN (6810)  AND a.Bod_id = ?  AND CONVERT(VARCHAR, Env_fec, 23) < '2022-02-06'
			   GROUP BY a.Pro_idint
			   ) AS detalle ON detalle.Pro_idint = a.Pro_idint
             WHERE a.Pro_idint IN (6810) AND Bod_id = ?";

    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array('09','09'));
    $productos = $q0->fetchAll();
    Database::disconnect_sqlsrv();
    return $productos;
  }
  static function get_coneteo_facturas_by_dia(){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.Pro_idint, a.Bod_id, a.Prd_cos, a.Prd_exi, detalle.TOTAL
             FROM APP_POS.dbo.PRODUCTO_DETALLE a
  ...
  static function get_facturas_seleccionadas($arreglo){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.orden_compra_id, a.factura_serie AS serie, a.factura_num AS factura, a.factura_total AS monto, a.proveedor_id, b.Prov_nom
            FROM docto_ped_pago a
            LEFT JOIN APP_POS.dbo.PROVEEDOR b ON a.proveedor_id = b.Prov_id COLLATE Modern_Spanish_CI_AS
            WHERE a.orden_compra_id IN $arreglo";
  ...
  static function getFacturasConteoByEstado($estado, $year){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT COUNT(orden_compra_id) AS conteo FROM docto_ped_pago WHERE id_status = ? AND YEAR(asignado_en) = ?";

    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array($estado,$year));
    $facturas = $q0->fetch();
    Database::disconnect_sqlsrv();
    return $facturas;
  }

  static function getOficiosFacturasByCompras($estado, $year){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.oficioId, a.oficionNum, a.oficioFecha, a.oficioDescripcion, a.oficioFacturas, a.operadoPor, a.operadoEn, a.direccionId, a.personaId,
                    b.descripcion AS direccion, c.descripcion_corta AS estado, a.oficioStatus,
                    CASE
                    WHEN movimientoId = 0 THEN 'Pendiente'
                    ELSE 'Finalizado'
                    END AS movimiento,
                    d.primer_nombre AS fnombre, d.segundo_nombre AS snombre, d.tercer_nombre AS tnombre, d.primer_apellido AS fapellido, d.segundo_apellido AS sapellido, d.tercer_apellido AS tapellido
             FROM DoctoPedPagoOficioEnvioDir a
             INNER JOIN rrhh_direcciones b ON a.direccionId = b.id_direccion
             INNER JOIN tbl_catalogo_detalle c ON a.oficioStatus = c.id_item
             LEFT JOIN rrhh_persona d ON a.personaId = d.id_persona
  ...
  static function getOrdenDetalleById($id_pago){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.id_pago,a.id_tipo_pago,a.nro_registro,a.id_year,a.creado_en,a.creado_por,a.cur_compromiso,a.cur_compromiso_r,a.cur_compromiso_fecha,a.cur_devengado,
            a.cur_devengado_r,a.cur_devengado_fecha,a.id_status,a.id_bitacora,a.monto
            FROM docto_ped_pago_presupuesto AS a
            WHERE a.id_pago = ?
    ...
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array($id_pago));
    $ordenDetalle = $q0->fetch();
    Database::disconnect_sqlsrv();
    return $ordenDetalle;
  }

  static function getFacturasByOrden($id_pago){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT * FROM DoctoPedPagoPresupuestoDetalle
            WHERE idPago = ?
  ...
  function retornaTipoOrden($valor){
    $color = array(0 => "warning", 1 => "info", 2 => "success", 3=> "danger");
    $icono = array(0 => "pause",1 => "arrow-right", 2 => "arrow-left", 3=> "times-circle");
    $tipo_pago = array(1 => "Orden de Compra", 2 => "COM-DEV", 3=> "CYD", 4 => "100", 5 => "100");
    $array = array(
      'icono'=>$icono[$valor],
      'color'=>$color[$valor],
      'tipo_pago'=>$tipo_pago[$valor]
    );
    return $array;
  }

  function retornaTipoIngreso($valor){
    $tipo_ingreso = array(1 => "Factura", 2 => "Cotización", 3 => "Recibo", 4 => "Formulario SAT");
    $array = array(
      'id_tipo_ingreso'=>$tipo_ingreso[$valor]
    );
    return $array;
  }

  function retornaTipoPago($valor){
    $id_tipo_pago = array(
      0 => "",
      1 => " - Cheque",
      2 => " - Transferencia"
    ...
    $array = array(
      'id_tipo_pago'=>$id_tipo_pago[$valor]
    );
    return $array;
  }
  function retornaEstadoCheque($valor){
    $estado_cheque = array(
      1 => "Factura",
      2 => "Viáticos"
    );
    ...
    $array = array(
      'estado_cheque'=>$estado_cheque[$valor]
    );
    return $array;
  }


  function retornaEstadoOrdenCompra($valor,$estado){
    /*$estado_orden = array(
      0 => "Registrado",
      1 => "Registrado",
      2 => "Entregado a presupuesto",
      3 => "Recibido presupuesto",
      4 => "Rechazado",
      5 => "Aprobado",
      6 => "Liquidación ",
      7 => "Entregado a presupuesto",
      8 => "Subdirección",
      9 => "Entregado CUR de aprobación",
      9 => "Entregado CUR de aprobación",
    );
...
*/
  ...
  function retornaEstadoOrdenCompraSeguimiento($valor){
    $estado_seguimiento = array(
      1 => "Revisión",
      2 => "En presupuesto",
      3 => "Enviado a compras",
      4 => "Sin cuota",
      5 => "Trámite de liquidación",
      6 => "Liquidación generada ",
      7 => "Entregado a presupuesto",
      8 => "Subdirección",
      9 => "Entregado CUR de aprobación"
    );
    ...
    $array = array(
      'estado_seguimiento'=>$estado_seguimiento[$valor]
    );
    return $array;
  }
  ...
  static function get_facturas_reporte($year)
  {
    $fitro='';

    $campoExtra = ",i.cur_compromiso_pre, i.cur_devengado_pre";
    ...
    $sql0 = "SELECT a.orden_compra_id, a.nro_orden, a.year, a.tipo_pago, a.status, a.asignado_en, a.asignado_por,b.ped_tra, a.reng_num,
            ...
            WHERE YEAR(a.asignado_en) IN ($year)
            ";
            ...
            if($cheque == 0 || $cheque == 2){
              ...
              /*if($tipo == 1){ //deuda
                $sql0 .= "WHERE YEAR(a.asignado_en) IN ($year) AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.status = 1";
                $sql0.=$filtro;
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
              ...
              if($tipo == 1 || $tipo == 2){ //liquidadas acreditamiento
                //$sql0 .= " WHERE YEAR(a.asignado_en) IN ($year) AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND a.cur_devengado > 0 AND a.status = 1";
                $sql0 .="LEFT JOIN (
                 SELECT a.cur_compromiso AS cur_compromiso_pre,
                  a.cur_devengado AS cur_devengado_pre, a.id_pago
            ...
            $sql0 .=" AND ISNULL(a.tipo_pago,0) IN (1,2) AND ISNULL(a.modalidad_pago,0) IN (0,2)";
            $sql0.=" ORDER BY a.orden_compra_id DESC";
    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array());
  ...
  static function getTotalPorCuatrimestreInsumo($insumo){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = " SELECT  a.Ppr_id, SUM(ISNULL(a.Ppr_costo,0) * a.Ppr_can) AS gastado, ((datepart(month,f.factura_fecha)-1)/4)+1 AS cuatrimestre
             FROM
			 ...
			 AND ((datepart(month,f.factura_fecha)-1)/4)+1 = ((datepart(month,GETDATE())-1)/4)+1
			 AND ISNULL(f.ped_nog,'') = ''
       AND YEAR(f.factura_fecha) = 2024 --AND g.id_year = ?
			 GROUP BY a.Ppr_id, ((datepart(month,f.factura_fecha)-1)/4)+1";

    //if($e['id_de'])
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array($insumo,date('Y')));
    $gastado = $q0->fetch();
    Database::disconnect_sqlsrv();
    return $gastado;
  }
