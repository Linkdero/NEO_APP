...
if (function_exists('verificar_session') && verificar_session() == true) {
  ...
  try{
    ...
    $sqlmail = "SELECT b.id_persona, b.persona_user
              FROM rrhh_persona_usuario b
              WHERE b.id_persona = ?";
    $qmail = $pdo->prepare($sqlmail);
    $qmail->execute(array($creador));
    $emisor = $qmail->fetch();
    ...
    $body.= '<br><br>DETALLE: '.$observaciones;
    ...
    $pdo->commit();
    $yes = array('msg'=>'OK','valor_nuevo'=>$observaciones, 'message' => $message, 'mail'=>$mail);
    //echo $body;
