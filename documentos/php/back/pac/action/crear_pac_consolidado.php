...
if (function_exists('verificar_session') && verificar_session() == true) {
  ...
  $id_nombre_consolidado = (!empty($_POST['id_nombre_consolidado'])) ? $_POST['id_nombre_consolidado'] : NULL;
  $id_descripcion_consolidado = (!empty($_POST['id_descripcion_consolidado'])) ? $_POST['id_descripcion_consolidado'] : NULL;


  $meses = $_POST['months'];
  $planes = $_POST['id_planes'];

  $parametros = substr($planes, 1);

  $arrayPlanes = explode(",",$parametros);
  ...
  try{
    ...
    $sql0 = "INSERT INTO APP_POS.dbo.PAC_CONSOLIDADO_E (pac_correlativo,pac_nombre,pac_detalle,pac_ejercicio_fiscal,pac_ejercicio_ant,pac_ejercicio_ant_desc,pac_renglon,pac_monto,creado_por,creado_en)
    VALUES(?,?,?,?,?,?,?,?,?,?)";
    $q0 = $pdo->prepare($sql0);
    $q0->execute(array(
      1,$id_nombre_consolidado,$id_descripcion_consolidado,2024,NULL,NULL,$id_renglon,0,$creador,date('Y-m-d H:i:s')
    ));
    ...
    /*foreach($meses AS $m){
      //if($m['checked']== 'true'){
        $sql2 = "INSERT INTO APP_POS.dbo.PAC_CONSOLIDADO_MES (pac_consolidado_id,pac_id_mes,cantidad,monto,id_status)
        values(?,?,?,?,?)";
        $q2 = $pdo->prepare($sql2);
    ...
    foreach($arrayPlanes AS $value){
      $sql2 = "INSERT INTO APP_POS.dbo.PAC_CONSOLIDADO_D (pac_consolidado_id,pac_id,id_status)
        values(?,?,?)";
        $q2 = $pdo->prepare($sql2);
        $q2->execute(array($cgid,$value,1));

        $sqlc = "UPDATE APP_POS.dbo.PAC_E SET pac_consolidado_status = ?
          WHERE pac_id = ?";
          $qc = $pdo->prepare($sqlc);
          $qc->execute(array(1,$value));
    }
    ...
    $valor_nuevo = array(
      'pac_consolidado_id'=>$cgid,
      'campo'=>'',
      'fecha'=>date('Y-m-d H:i:s'),
      'valor'=>'Se agregó un nuevo Plan de Compras Consolidado'
    );

    /*$log = "VALUES(325, 8017, 'APP_POS.dbo.PAC_CONSOLIDADO_E', '".json_encode($valor_anterior)."' ,'".json_encode($valor_nuevo)."' , ".$_SESSION["id_persona"].", GETDATE(), 3)";
