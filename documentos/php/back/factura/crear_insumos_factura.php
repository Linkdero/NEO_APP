...
if (function_exists('verificar_session') && verificar_session() == true) {
  ...
  $pedTra = (!empty($_POST['ped_tra'])) ? $_POST['ped_tra'] : NULL;
  $tipo = $_POST['tipo'];
    ...
    if($tipo == 1){
      ...
      foreach($insumos AS $i){
        ...
        if($i['checked']== 'true'){
          ...
          values(?,?,?,?,?,?,?)";
          $q2 = $pdo->prepare($sql2);
          $q2->execute(array($orden_id, $x, $i['Ppr_id'],$i['v_rec'],1,$i['precio_unitario'],$i['Ped_tra']));
      ...
      foreach($insumos AS $i){
        ...
        if($i['checked']== 'true'){
          //$ped_tra = $i['Ped_tra'];
          //echo $i['v_rec']+$i['Pedd_canf'].' |-| ';
          $sql2 = "UPDATE docto_ped_pago_detalle SET Ped_tra = ? WHERE orden_compra_id = ? AND Ppr_id = ?";
          $q2 = $pdo->prepare($sql2);
          $q2->execute(array($pedTra,$orden_id, $i['Ppr_id']));

          $sql3 = "UPDATE APP_POS.dbo.PEDIDO_D SET Pedd_canf = ISNULL(Pedd_canf,0) + ? WHERE Ped_tra=? AND Ppr_id = ?";
          $q3 = $pdo->prepare($sql3);
          $q3->execute(array($i['Ppr_can'],$pedTra, $i['Ppr_id']));
