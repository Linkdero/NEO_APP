...
if (function_exists('verificar_session') && verificar_session() == true) :
    ...
    if($facturas == true){
      $facturas = $clased->get_facturas_reporte($year);
    }
    ...
    //echo $f_final;
        ...
        if($f['id_tipo_ingreso'] == 1 || $f['id_tipo_ingreso'] == 2){
          ...
          if($f['factura_publicada'] == 0){
            $p_publicar.=/*($diferencia > 0) ?*/'<h5 class="text-danger"><i class="fa fa-times-circle"></i> Pendiente de publicar</h5>';// : '';
          }
          //fin
        }
        if(!empty($f['factura_publicada'])){

          $p_publicar = '<div style="width:150px"><h5 class="text-info"> <i class="fa fa-check-circle "></i> Factura publicada</h5></div>';
        }else if($f['factura_publicada'] == 0 ){
          $p_publicar = '<div style="width:150px"> <h5 class="text-danger"><i class="fa fa-check-circle"></i> Factura pendiente de publicar</h5></div>';
          $opciones.='1';
          $fecha_estimada=' Último dia para publicar: <br><strong>'.fecha_dmy($f_final).'</strong> ';//'<br> Le quedan: <span class="'.$class.'">'.$diferencia.'</span> para publicar<br>';
        }
        else{
        ...
        if($estado == 'Anulada'){
          ...
          $p_asignart.= ((!empty($f['primer_nombre']))) ? '<h5 class="text-info"><i class="fa fa-check-circle"></i> Asingado a técnico</h5>' : '<h5 class="text-danger"><i class="fa fa-times-circle"></i> Pendiente de asingar a técnico</h5>';
          $opciones.= ((!empty($f['primer_nombre']))) ? '' : '2';
          $p_asignarr.= (!empty($f['clase_proceso'])) ? '<h5 class="text-info"><i class="fa fa-check-circle"></i> Clase de proceso</h5>' : '<h5 class="text-danger"><i class="fa fa-times-circle"></i> Pendiente de asignar clase de proceso</h5>';
          $opciones.= (!empty($f['clase_proceso'])) ? '' : '3';
        }
        ...
        $sub_array = array(
          ...
          'action'=>'<span class="btn btn-sm btn-soft-info" data-toggle="modal" data-target="#modal-remoto-lgg2" href="documentos/php/front/factura/factura_detalle.php?orden_id='.$f['orden_compra_id']. '&ped_num=' .$f['Ped_num'].  '"><i class="fa fa-pencil-alt"></i></span>',
          ...
          'clase_proceso'=>(!empty($clase_proceso['tipo_pago'])) ? $clase_proceso['tipo_pago'] : '',//$clase_proceso,
          ...
          'days'=>$i,
          'class'=>$class_yellow,
          'dias'=>'<div style="width:100px"><strong><h2 class="'.$class2.'">'.$i.'</h2></strong></div>',
          'fecha_estimada'=>$fecha_estimada,//. $fecha_ini .' | | '.$f_final
          'tipo_ingreso'=>$clased->retornaTipoIngreso($f['id_tipo_ingreso']),
          'opciones'=>$opciones,
          'p_publicar'=>$p_publicar,
          'p_asignart'=>$p_asignart,
          'p_asignarr'=>$p_asignarr,
        );
        $data[] = $sub_array;
      }
