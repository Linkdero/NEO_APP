    ...
    $sql0 = "SELECT a.ped_tra,REPLACE(STR(a.ped_num, 5), SPACE(1), '0') AS ped_num,a.ped_fec, a.ped_obs, c.Dir_com AS direccion, b.Uni_Nom AS departamento,
             ...
             if($tipo == 0){
               ...
               if ($u->hasPrivilege(301)){
                 ...
                 $sql0.="AND a.Ped_status IN (8156,8139) OR ISNULL(a.Ped_status,0) = 0 AND YEAR(a.ped_fec) = $fecha";
                 //echo '1';
...
";
  ...
  static function get_pedidos_by_estado($estado){
    ...
    $sql0 = "SELECT d.Ped_tra, REPLACE(STR(d.Ped_num, 5), SPACE(1), '0') AS ped_num, d.Ped_fec, e.descripcion AS estado,
          ...
          INNER JOIN APP_POS.dbo.DIRECCION i ON h.Dir_cor = i.Dir_cor
          --WHERE CONVERT(varchar, f.ped_seguimiento_fecha, 23) BETWEEN ? AND ?
          WHERE YEAR(f.ped_seguimiento_fecha) = ? AND e.ped_tipo_seguimiento_id = ? ";
          if($estado == 8148){
  ...
  static function get_pedidos_seleccionados($valores)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT REPLACE(STR(Ped_num, 5), SPACE(1), '0') AS ped_num, Ped_obs, CONVERT(VARCHAR, Ped_fec, 105) AS Ped_fec
    ...
    $sql0 = "SELECT a.orden_compra_id, a.nro_orden, a.year, a.tipo_pago, a.status, a.asignado_en, a.asignado_por,b.ped_tra, a.reng_num,
            ...
            if($cheque == 0){
              ...
              if($tipo == 4){ //liquidadas acreditamiento
                $sql0 .= " WHERE YEAR(a.asignado_en) IN ($year) AND a.factura_publicada = 1 AND ISNULL(a.cur,0) = 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.status = 1";
                if($year == 2022){
                  $sql0 .= " AND MONTH(a.asignado_en) > '04'";
                }
              ...
              if($tipo == 0){//pendientes
                $sql0 .= "WHERE YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.cheque_nro,0) = 0 AND ISNULL(a.factura_publicada,0) = 0
                          OR YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.cheque_nro,0) > 0 AND ISNULL(a.factura_publicada,0) = 0";
              }else
              if($tipo == 4){ //deuda
                $sql0 .= "WHERE YEAR(a.asignado_en) = $year AND ISNULL(a.modalidad_pago,0) = 1 AND a.status = 1 AND ISNULL(a.factura_publicada,0) = 1";
                //$sql0 .= "WHERE YEAR(a.asignado_en) IN (2022,2023) AND MONTH(a.asignado_en) > '04' AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.status = 1";
              }else if($tipo == 3){
  ...
  static function get_ordenes_group_pendientesbk($tipo)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT nro_orden, clase_proceso, year, cur, cur_devengado, SUM(factura_total) AS total FROM docto_ped_pago
  ...
  static function get_ordenes_group_pendientes($tipo,$year)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
LEFT JOIN (SELECT a.Pro_idint, SUM(a.Envd_can) AS TOTAL FROM APP_POS.dbo.ENVIO_D a
  ...
  function retornaTipoIngreso($valor){
    $tipo_ingreso = array(1 => "Factura", 2 => "Cotización");
    $array = array(
