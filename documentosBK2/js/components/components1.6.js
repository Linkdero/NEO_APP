    ...
    getInsumos: function(){
      axios.get('documentos/php/back/pedido/get_insumos_by_pedido', {
        ...
        $.each(response.data,function(pos, elemento){
          total+=parseInt(elemento.lineas);
          console.log(elemento.lineas);
        });
        this.totalOcupado = total;
        console.log(total+ ' -- '+ this.totalOcupado);*/
    ...
    addNewRow() {
      ...
      if ($('#Ppr_id_').val() != null) {
        axios.get('documentos/php/back/pedido/get_insumo_seleccionado', {
          ...
          /*if (this.insumos.find((item) => item.Ppr_id == response.data.Ppr_id)) {
            Swal.fire({
              type: 'error',
              title: 'El insumo ya existe en la lista',
              showConfirmButton: false,
              timer: 1100
            });
          } else {*/
