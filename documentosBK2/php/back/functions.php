...
class documento
{
  ...
  static function get_pedidos_remesas_by_direccion($direccion, $tipo, $fecha)
  {
    ...
    $sql0 = "SELECT a.ped_tra,REPLACE(STR(a.ped_num, 5), SPACE(1), '0') AS ped_num,a.ped_fec, a.ped_obs, c.Dir_com AS direccion, b.Uni_Nom AS departamento,
             ...
             }else if($_SESSION['id_persona']== 5449){
               $direccion = 97;
               $sql0 .= "WHERE a.Uni_cor = ?  AND YEAR(a.ped_fec) = '$fecha'";
             }
             else {
...
";
  ...
  static function get_facturas($tipo,$tecnico)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.orden_compra_id, a.nro_orden, a.year, a.tipo_pago, a.status, a.asignado_en, a.asignado_por,b.ped_tra, a.reng_num,
                    ...
                    a.npg, f.descripcion_corta AS estado_factura, a.id_renglon, g.descripcion AS direccion
            ...
            if($tipo == 0){//pendientes
              $sql0 .= "WHERE YEAR(a.asignado_en) IN (2022,2023)  AND ISNULL(a.modalidad_pago,0) = 0 AND ISNULL(a.factura_publicada,0) = 0 AND a.status = 1";
              if($tecnico > 0){
                $sql0.=" AND d.id_persona = $tecnico ";
              }
              $sql0 .= "OR YEAR(a.asignado_en) IN (2022,2023)  AND ISNULL(a.modalidad_pago,0) = 2 AND ISNULL(a.cur,0) = 0 AND ISNULL(a.factura_publicada,0) = 0 AND a.status = 1";
              if($tecnico > 0){
                $sql0.=" AND d.id_persona = $tecnico ";
              }
            }else
            if($tipo == 1){ //deuda
              $sql0 .= "WHERE YEAR(a.asignado_en) IN (2022,2023)  AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.status = 1";
            }else
            if($tipo == 2){ //liquidadas acreditamiento
              $sql0 .= " WHERE YEAR(a.asignado_en) IN (2022,2023)  AND ISNULL(a.modalidad_pago,0) = 2 AND a.cur > 0 AND a.cur_devengado > 0 AND a.status = 1";
            }else
            if($tipo == 4){ //liquidadas acreditamiento
              $sql0 .= " WHERE YEAR(a.asignado_en) IN (2022,2023)  AND a.factura_publicada = 1 AND ISNULL(a.cur,0) = 0 AND ISNULL(a.cur_devengado,0) = 0 AND a.status = 1";
            }else if($tipo == 3){
              $sql0 .= " WHERE YEAR(a.asignado_en) IN (2022,2023)  AND a.status = 5";
            }
  ...
  static function get_insumos_by_factura($orden)
  {
    ...
    $sql0 = "SELECT a.orden_compra_id, a.reng_num, a.Ppr_id, a.Ppr_can, a.Ppr_status,
             ...
             INNER JOIN APP_POS.dbo.PPR b ON a.Ppr_id=b.Ppr_id
             INNER JOIN APP_POS.dbo.MEDIDA c ON b.Med_id = c.Med_id
             WHERE a.orden_compra_id=? AND a.Ppr_status = 1
  ...
  static function get_ordenes_group_pendientes($tipo)
  {
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.id_pago, a.nro_registro, a.id_tipo_pago,  a.cur_compromiso, a.cur_devengado, b.total, 0 as nro_orden
             ...
             LEFT JOIN (
               SELECT SUM(factura_total) AS total, id_pago FROM docto_ped_pago
               WHERE  status = 1
               GROUP BY  id_pago
             ) AS b ON b.id_pago = a.id_pago
             WHERE YEAR(a.creado_en) >= '2022' AND MONTH(a.creado_en) > '04' ";

            if($tipo == 0){ //sin cur
              $sql0 .= 'AND ISNULL(a.cur_compromiso,0) = 0 AND ISNULL(a.cur_devengado,0) = 0 ';
            }else
            if($tipo == 1){ //cur compromiso
              $sql0 .= 'AND ISNULL(a.cur_compromiso,0) > 0 AND ISNULL(a.cur_devengado,0) = 0 ';
            }
...
LEFT JOIN (SELECT a.Pro_idint, SUM(a.Envd_can) AS TOTAL FROM APP_POS.dbo.ENVIO_D a
  ...
  static function get_facturas_seleccionadas($arreglo){
    $pdo = Database::connect_sqlsrv();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql0 = "SELECT a.orden_compra_id, a.factura_serie AS serie, a.factura_num AS factura, a.factura_total AS monto
            FROM docto_ped_pago a
