...
set_time_limit(0);
  ...
  if($estado_id != 8170){
    ...
    //12 presupuesto
    //13 tesoreria
    if($privilegio[8]['flag_es_menu'] == 1 || $privilegio[3]['flag_es_menu'] == 1 || $um->accesoModulo(7851)){
      if($privilegio[8]['flag_autoriza']==1 && $privilegio[8]['flag_descarga'] == 1 || $privilegio[8]['flag_actualizar'] || $um->accesoModulo(7851)){

        if($estado_id==0){
          $verificacion=1;
        }
        if($estado_id==8156 && empty($asig['id_persona']) && $privilegio[8]['flag_descarga'] == 1){
    ...
    if($privilegio[6]['flag_es_menu'] == 1 || $um->accesoModulo(7851)){
      //subsecretaría
      if($privilegio[6]['flag_autoriza'] == 1 || $um->accesoModulo(7851)){
      ...
      if($privilegio[2]['flag_autoriza'] == 1  || $um->accesoModulo(7851)){
      ...
      if($privilegio[2]['flag_actualizar'] == 1 && $_SESSION['id_persona']==$asig['id_persona'] || $um->accesoModulo(7851)){
        if($estado_id == 8164 && $e['Ped_bitacora_id']==8145){// revisión compras
          $verificacion=10;
        }
