$(document).ready(function(){
  window.addEventListener("afterprint", imprimir_anticipo);
})
...
function nombramiento_definitivo_reporte(id_nombramiento, dia, mes, year,id_empleado) {
...
function cabeceras_nombramiento(doc, punto, direccion, correlativo, nombramiento, fecha, motivo, hojas, fecha_ini, fecha_fin, duracion, beneficios, funcionario, tipo, incremental, t_hojas, observaciones, destinos, descripcion_lugar) {
  ...
  if (incremental <= t_hojas) {
    ...
    doc.writeText(5, 269, 'PBX: 2327 - 6000 FAX: 2327 - 6000', { align: 'center', width: 209 });
...
function imprimir_con_fecha(nombramiento, tipo, id_empleado) {
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      if (tipo == 1) {
        imprimir_anticipo(nombramiento, dia, mes, year,id_empleado);
      } else if (tipo == 2) {
        imprimir_liquidacion(nombramiento, dia, mes, year, id_empleado);
      } else if (tipo == 3) {
        imprimir_informe(nombramiento, dia, mes, year,id_empleado);
      } else if (tipo == 4) {
        nombramiento_definitivo_reporte(nombramiento, dia, mes, year,id_empleado);
      } else if (tipo == 5) {
        imprimir_complemento(nombramiento, dia, mes, year, id_empleado);
      } else if (tipo == 6) {
...
function imprimir_anticipo(nombramiento, dia, mes, year,id_empleado) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.data.length; i++) {
        ...
        if (data.data[i].estado_viatico == 0) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.data.length; i++) {
        ...
        doc.setFontType('normal');
        doc.writeText(181.5, punto2 + 114, data.data[i].total_real, { align: 'right', width: 25 });
        doc.writeText(181.5, punto2 + 120, data.data[i].otros_gastos, { align: 'right', width: 25 });
        doc.setFontType("bold");
        doc.writeText(181.5, punto2 + 126, data.data[i].total_real_total, { align: 'right', width: 25 });
        //doc.writeText(150, punto2+116 ,'FECHA:',{align:'left',width:65});
        ...
        doc.writeText(181.5, punto2 + 138, data.data[i].monto_anticipo, { align: 'right', width: 25 });
        doc.writeText(181.5, punto2 + 144, (data.data[i].total_reintegro != '') ? data.data[i].total_reintegro : '0.00', { align: 'right', width: 25 });
        doc.writeText(181.5, punto2 + 150, data.data[i].a_favor, { align: 'right', width: 25 });
        doc.setLineWidth(1);
        doc.setFontType("bold");
        doc.line(5, punto2 + 152, 211, punto2 + 152);
        doc.writeText(10, punto2 + 156, 'TOTAL', { align: 'left', width: 205 });
        doc.writeText(181.5, punto2 + 156, data.data[i].total_, { align: 'right', width: 25 });
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.data.length; i++) {
        ...
        if (data.data[i].estado_viatico == 0) {
          ...
          imprimir_constancia_vacia(2, doc);
        }
        doc.setTextColor(33, 33, 33);
        doc.setFontType("normal");
        punto = 10;
...
function imprimir_constancia_vacia(tipo,docto) {
  var documento;
  var doc = '';
  punto = 10;
  if(tipo == 1){
...
function imprimir_informe(id_nombramiento, dia, mes, year, id_empleado) {
      ...
      if (data.data[0].tipo == 1) {
        ...
        for (var h = 0; h <= t_hojas; h++) {
          ...
          if (pages > 0) {
            ...
            doc.writeText(5, 265, 'PBX: 2327 - 6000 FAX: 2327 - 6000', { align: 'center', width: 209 });
...
function imprimir_resumen(id_nombramiento) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var nombramiento = '', correlativo, direccion, funcionario = '', motivo = '', fecha, fecha_ini, fecha_fin, duracion, funcionario,observaciones;
      //punto +=0;
      //console.log(data);
      doc.setFontType("normal");
      doc.setFontSize(9);
      ...
      for (var i = 0; i < registros; i++) {
        ...
        var r_lineas2 = doc.splitTextToSize(observaciones, 160);
        punto+=5;
        doc.text(10, punto, r_lineas2);
...
function imprimir_complemento(nombramiento, dia, mes, year, id_empleado) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        doc.writeText(5, 35, 'LIQUIDACIÓN DE VIÁTICOS EN COMISIÓN OFICIAL SAAS/' + data[i].nombramiento, { align: 'center', width: 205 });
        doc.setFontSize(9);
        doc.setFontType('normal');
        ...
        doc.writeText(45, punto - 8, 'FECHA', { align: 'center', width: 7 });
        doc.writeText(63, punto - 8, 'EMPLEADO', { align: 'left', width: 65 });

        doc.setFontSize(6);
        doc.writeText(125, punto - 8, 'ANTICIPO', { align: 'center', width: 28 });
        doc.writeText(143, punto - 8, 'REINTEGRO', { align: 'center', width: 28 });
        doc.writeText(161, punto - 8, 'COMPLEMENTO', { align: 'center', width: 28 });
        doc.writeText(179, punto - 8, 'TOTAL VL', { align: 'center', width: 28 });
        doc.setFontSize(9);
        var suma_a = 0;
        ...
        doc.writeText(38, punto, data[i].fecha_liquidacion, { align: 'center', width: 28 });
        doc.writeText(63, punto, data[i].empleado, { align: 'left', width: 65 });
        doc.writeText(118, punto, (data[i].bln_anticipo == 1) ? data[i].m_p : '--', { align: 'right', width: 28 });
        doc.writeText(136, punto, data[i].reintegro, { align: 'right', width: 28 });
        doc.writeText(154, punto, data[i].complemento, { align: 'right', width: 28 });
        doc.writeText(172, punto, data[i].m_r, { align: 'right', width: 28 });
        ...
        doc.writeText(130, punto+50 ,'TOTAL A RECIBIDO:',{align:'left',width:65});
        doc.setFontType('bold');
        ...
        doc.writeText(0, punto + 50, (data[ultimo_registro].total_recibido), { align: 'right', width: 200 });



        var r_d1 = `TOTAL GASTADO **${data[ultimo_registro].total_complemento_letras}** `;
        multilineaConNegrita(20, punto + 60, 190, r_d1, doc);

        doc.writeText(20, punto+90 ,'FIRMA:',{align:'left',width:65});
...
function imprimir_monto_por_nombramiento(nombramiento) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var totalpaginas = registros / 12;

      if(paginas < 1){
        pages = 1;
        paginas = 1;
      }else{
        var decimals =  totalpaginas - Math.floor(totalpaginas);
        var residuo = decimals.toFixed(1);

        pages = totalpaginas - decimals;
        paginas = pages;
        console.log('ALKJDFÑLKAJD :::: ' +pages);

        if (decimals > 0) {
          pages += 1;
          paginas += 1;
          console.log('alkdfjla '+pages);
        }
      }
      t_hojas = pages;
      console.log(pages);
      ...
      for (var p = 0; p < paginas; p++) {
        ...
        if (p < paginas) {
          ...
          doc.writeText(5, 35, 'COMISCIÓN No. '+ nombramiento, { align: 'center', width: 205 });
          doc.setFontSize(9);
          doc.setFontType('normal');
