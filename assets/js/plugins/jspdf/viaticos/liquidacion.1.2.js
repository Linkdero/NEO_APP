  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if(info.nombre_mes_ini == info.nombre_mes_fin){
        ...
        if(info.nombre_year_ini == info.nombre_year_fin){
          textoRango = `DEL ${info.dia_ini}  DE ${info.nombre_mes_ini} AL ${info.dia_fin} DE ${info.nombre_mes_fin} DE ${info.year_comision}`;
        }else{
          textoRango = `DEL ${info.dia_ini}  DE ${info.nombre_mes_ini} DEL ${info.nombre_year_ini} AL ${info.dia_fin} DE ${info.nombre_mes_fin} DE ${info.nombre_year_fin}`;
        }
        ...
        success: function (data) {
          ...
          var reformattedArray = data.data.map(function(rObjPagina){
            ...
            var reformattedArray2 = rObjPagina.facturas.map(function(obj){
              dia_factura = obj.dia_id;
              tipo_factura = obj.factura_tipo;
              ...
              if(rObjPagina.dia_id == obj.dia_id  && rObjPagina.factura_tipo == obj.factura_tipo){
                ...
                if((rObjPagina.factura_tipo == obj.factura_tipo) && obj.flag_error == 1){

                  var obs = "";
                  if(rObjPagina.dia_observaciones_al.length > 0){
                    obs = `${rObjPagina.dia_observaciones_al}`;
                  }else if(rObjPagina.dia_observaciones_hos.length > 0){
                    obs = `${rObjPagina.dia_observaciones_hos}`;
                  }
                  error1 += 1;
                  desc1 = ` **NOTA ACLARATORIA:** ${obs} ${tipoDocto} No.: ${obj.factura_numero} DE ${obj.proveedor} FUE IMPRESA CON FECHA: ${obj.fecha}, `;
                }else if((rObjPagina.factura_tipo == obj.factura_tipo) && obj.flag_error == 0 && facturaTextoE == ''){
                  var obs = "";
                  if(rObjPagina.dia_observaciones_al.length > 0 && rObjPagina.factura_tipo == 1){
                    desc2 = `${rObjPagina.dia_observaciones_al}`;
                  }else if(rObjPagina.dia_observaciones_hos.length > 0 && rObjPagina.factura_tipo == 2){
                    desc2 = `${rObjPagina.dia_observaciones_hos}`;
                  }
                  error2 = 1;
                }
               }
            ...
            facturaTextoE = (error1 > 0) ? desc1 : desc2;
            ...
            /*if(rObjPagina.factura_tipo == 1 && rObjPagina.dia_id == dia_factura && rObjPagina.factura_tipo == tipo_factura){
              if(rObjPagina.dia_observaciones_al.length > 0){
                doc.setFontType('bold');
            ...
            if(rObjPagina.factura_tipo == 2 && rObjPagina.dia_id == dia_factura && rObjPagina.factura_tipo == tipo_factura){
              if(rObjPagina.dia_observaciones_hos.length > 0){
                doc.setFontType('bold');
                facturaTexto += ' **NOTA ACLARATORIA:** ' + rObjPagina.dia_id +' || || ' +dia_factura;
