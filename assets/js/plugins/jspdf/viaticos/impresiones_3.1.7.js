...
function imprimir_con_fecha(nombramiento,tipo){
  var f = new Date();
  ...
  if (result.value) {
    ...
    if(tipo==1){
      ...
      imprimir_liquidacion(nombramiento,dia,mes,year);
    }else if(tipo==3){
...
function imprimir_liquidacion(nombramiento,dia,mes,year){
...
function imprimir_constancia(nombramiento){
  $.ajax({
    type: "POST",
          ...
          if(pages>0){
            ...
            //if(pages==t_hojas){
              ...
              //if(registros==1){
                ...
                doc.text(25, punto+20, 'Estimado Señor: ');
                //doc.text(60, punto+28, 'M ');
                var r_d1 = 'Me dirijo a usted para informarle que  de conformidad con el Nombramiento número '+data.data[i].nombramiento+' de fecha '+data.data[i].dia_nombramiento+ ' de '+data.data[i].mes_nombramiento+' de '+data.data[i].year_nombramiento+'. Me constituí en '+data.data[i].lugar+ ' del '+data.data[i].dia_salida +' de '+data.data[i].mes_salida +' de '+data.data[i].year_salida +' al '+data.data[i].dia_regreso +' de '+data.data[i].mes_regreso +' de '+data.data[i].year_regreso +' para '+data.data[i].motivo +' de conformidad con el horario siguiente: ' ;
                var r_lineas1 = doc.splitTextToSize(r_d1, 170);
                doc.text(25, punto+28, r_lineas1);
              /*}else{
                //doc.text(25, 63, 'Señores: ');
            ...
            doc.setTextColor(68, 68, 68);
            doc.text(30, punto+100, '(f)    ' +data.data[i].empleado);
            doc.text(30, punto+107, '** ULTIMA LINEA **');

			doc.setTextColor(0,0,0);
			doc.setFontType("bold");
			doc.setDrawColor(0,0,0);
            doc.line(30, punto+100+2, 150, punto+100+2);
			doc.setFontType("normal");
            doc.setTextColor(0, 0, 0);
...
function imprimir_complemento(nombramiento,dia,mes,year){
    ...
    $.ajax({
      ...
      success:function(data){
        ...
        doc.writeText(5,35,'RESUMEN DE GASTOS DEL NOMBRAMIENTO SAAS/'+data[0].nombramiento,{align:'center',width:205});
        doc.setFontSize(9);
        doc.setFontType('normal');
    ...
    $.ajax({
      ...
      success:function(data){
        ...
        var doc = new jsPDF('p','mm');
            ...
            punto+=10;
            doc.writeText(25, punto+5 ,'Sr. (a)',{align:'left',width:100});
            doc.writeText(25, punto+10 ,data.director,{align:'left',width:100});
            doc.writeText(25, punto+15 ,data.direccion,{align:'left',width:100});
            doc.writeText(25, punto+20 ,'SU DESPACHO',{align:'left',width:100});

            doc.writeText(25, punto+30 ,'Sr.(a) Director (a)',{align:'left',width:100});
            punto+=35
            for(var i = 0; i <8; i++){
              punto+=10;
