  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.data.length; i++) {
        ...
        doc.setFontSize(7.5);
        var r_d1 = '';
        if (Number(data.data[i].form) <= 45000) {
          r_d1 = data.data[i].resolucion;
        } else {
          r_d1 = `AUTORIZADO POR LA CONTRALORIA GENERAL DE CUENTAS SEGUN RESOLUCION No. F.O. –JM-47-2022C 000983 GESTIÓN NÚMERO: 644611 DE FECHA 09-03-2022 ` +
            `DE CUENTA S1-22 · 15000 Formulario de Viatico Anticipo en Forma Electrónica DEL No. 45001 AL 60000 SIN SERIE No. CORRELATIVO Y FECHA DE ` +
            `AUTORIZACION DE IMPRESION 299-2022 DEL 27-04-2022 · ENVIO FISCAL 4-ASCC 19645 DEL 27-04-2022 LIBRO 4-ASCC FOLIO 157 `;
          ...
          if (pages > 0) {
            ...
            if (data.data[i].id_pais == 'GT') {
              if (data.data[i].descripcion_lugar == 0 || data.data[i].descripcion_lugar == 1) {
                ...
                doc.setFontType("normal");
                doc.text(150, punto + 45, data.data[i].hora_salida_saas);
                doc.text(170, punto + 45, data.data[i].fecha_salida);

                doc.text(25, punto + 50, 'Hora de llegada a : ');
                doc.text(55, punto + 50, data.data[i].lugar);
                doc.text(150, punto + 50, data.data[i].hora_llegada_lugar);
                doc.text(170, punto + 50, data.data[i].fecha_llegada_lugar);

                doc.text(25, punto + 55, 'Hora de Salida de : ');
                doc.text(55, punto + 55, data.data[0].lugar);
                doc.text(150, punto + 55, data.data[i].hora_salida_lugar);
                doc.text(170, punto + 55, data.data[i].fecha_salida_lugar);
